...
import { fmt, pct } from './utils/money.js'
...
// tab previously read ONLY the rec.biz summary â which is 0 / '' / 'Unknown' for a
...
function getAllRecords() {
  ...
  // could surface â and analyze â a different account's saved record.
...
function recDepreciation(rec) {
  ...
  // "No Depreciation Recorded" / "Vehicle â Review Before Filing" risk cards.
  // Previously this counted only pnl.depreciation (MACRS/bonus). A filer who took
  // Â§179 in the K-1 Box 11 field (box11_12) but left the depreciation field blank
  // â e.g. a 100%-business-use vehicle expensed under Â§179 â registered as zero,
  // so the scan falsely told them they had "no depreciation" and "no vehicle
  // deduction." Count box11_12 (Â§179) alongside MACRS/bonus depreciation.
...
function completeness(rec) {
  ...
  // UX-M6 FIX: penalise blank RE entity â an RE card with no rental income entered
...
function NoData({ tab = 'risk' }) {
  const tabInfo = {
    risk: {
      ...
      desc: 'Flags officer compensation issues, audit triggers, penalty exposure, passive-loss violations, and entity-structure risks â personalized to your numbers.',
    },
    optimize: {
      icon: 'ð¡',
      ...
      desc: 'Model a financial decision before making it. Try different salary levels, add a deduction, or max a retirement account â see the estimated dollar impact on your projected tax.',
    },
    compliance: {
      icon: 'ð',
      title: 'IRS Schedule Map',
      desc: 'Shows every form and schedule you\'ll need to file â with status (required / covered / action needed) based on your income structure.',
    },
    reports: {
      icon: 'ð',
      title: 'Reports & Tools',
  ...
  return (
    <div style={{ textAlign: 'center', padding: '48px 24px', background: '#F8FAFC', borderRadius: 14, border: '1px solid #E2E8F0' }}>
      ...
      <div style={{ fontSize: 12, color: '#94A3B8', marginBottom: 20, lineHeight: 1.5 }}>
        Complete Step 1 (business info) and Step 2 (personal return) in the Tax Tracker to unlock this â takes about 5 minutes.
      </div>
...
// ââ TAB 1: Risk Scan âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function RiskScan({ rec }) {
  ...
  if (sCorpEntities.length > 0) {
    sCorpEntities.forEach((e, ei) => {
      ...
      if (eOfficerSal === 0 && eK1 > 20000) {
        findings.push({ key: 'scorp-no-salary-' + ei, level: 'high', icon: 'ð¨', title: `No Officer Compensation â ${entityName} (Audit Risk)`,
          detail: `${entityName} shows ${fmt(eK1)} in K-1 income but no officer compensation recorded. Tax practitioners and case law (Watson v. Commissioner, 668 F.3d 1008) flag zero salary as one of the top S-Corp audit triggers. The IRS applies a facts-and-circumstances test â there is no published safe harbor percentage.`,
          action: `Set ${entityName}'s officer compensation on Step 1. A common practitioner starting point is 35â45% of total S-Corp compensation. The correct amount depends on your role, hours, industry, and comparable pay â discuss with your CPA.` })
      } else if (eOfficerSal > 0 && eK1 > 30000 && eOfficerSal / (eOfficerSal + eK1) < SCORP_REASONABLE_COMP_RATIO_THRESHOLD) {
        findings.push({ key: 'scorp-low-salary-' + ei, level: 'medium', icon: 'â ï¸', title: `Officer Compensation May Be Too Low â ${entityName}`,
          detail: `${entityName} shows ${fmt(eOfficerSal)} in officer compensation versus ${fmt(eK1)} in K-1 income (${((eOfficerSal/(eOfficerSal+eK1))*100).toFixed(1)}% of total S-Corp compensation). Tax practitioners commonly recommend a salary-to-total-compensation ratio of 35â45%, based on case law including Watson v. Commissioner, 668 F.3d 1008 (8th Cir. 2012). The IRS applies a facts-and-circumstances test â there is no published safe harbor percentage.`,
          action: `Consider increasing ${entityName}'s officer compensation to bring it within the 35â45% practitioner-recommended range. The correct amount depends on your role, hours, industry, and comparable pay â discuss with your CPA.` })
      } else if (eOfficerSal > 0) {
        findings.push({ key: 'scorp-salary-ok-' + ei, level: 'good', icon: 'â', title: `Officer Compensation Recorded â ${entityName}`,
        ...
        // during data entry. A salary that is within the 35â45% heuristic band (i.e. the
        // low-salary finding above did NOT fire) is still flagged here as an informational
        // reminder to document the basis for the salary â the IRS scrutinizes S-Corp comp
        ...
        if (eTotalComp > 20000) {
          ...
          findings.push({ key: 'scorp-comp-doc-' + ei, level: 'info', icon: 'ð',
            title: `Reasonable Compensation â Document the Basis (${entityName})`,
            detail: `Officer salary (${fmt(eOfficerSal)}) is ${eRatio}% of total S-Corp compensation. The IRS applies a facts-and-circumstances test under Rev. Rul. 74-44 â there is no published safe-harbor ratio. Watson v. Commissioner, 668 F.3d 1008 (8th Cir. 2012) is the leading case, but its fact pattern (12% ratio) is extreme; any salary below fair market value for the services rendered is at risk. S-Corp reasonable compensation is one of the most common examination triggers in IRS campaigns targeting pass-through entities.`,
    ...
    if (ownerComp === 0 && k1 > 20000) {
      findings.push({ level: 'high', icon: 'ð¨', title: 'No Officer Compensation â Audit Risk',
        ...
        action: 'Set reasonable W-2 officer compensation for the services you perform. There is no IRS safe-harbor percentage â reasonable compensation is a facts-and-circumstances determination â but a common practitioner starting point is 35â45% of total officer compensation (salary + distributions). The salary is deductible to the S-Corp and reduces self-employment tax exposure.' })
    } else if (ownerComp > 0 && k1 > 30000 && ownerComp / (ownerComp + k1) < SCORP_REASONABLE_COMP_RATIO_THRESHOLD) {
      findings.push({ level: 'medium', icon: 'â ï¸', title: 'Officer Compensation May Be Too Low',
        detail: `Reported owner compensation is ${fmt(ownerComp)} versus K-1 income of ${fmt(k1)} (${((ownerComp/(ownerComp+k1))*100).toFixed(1)}% of total compensation). Tax practitioners commonly recommend a salary-to-total-compensation ratio of 35â45%, based on case law including Watson v. Commissioner. The IRS applies a facts-and-circumstances test â there is no published safe harbor.`,
        action: `Consider increasing your salary to bring it within the 35â45% practitioner-recommended range. The correct amount depends on your role, hours, industry, and comparable pay â discuss with your CPA.` })
    } else if (ownerComp > 0) {
      findings.push({ level: 'good', icon: 'â', title: 'Officer Compensation Recorded',
      ...
      if (_legacyTotalComp > 20000) {
        ...
        findings.push({ level: 'info', icon: 'ð',
          title: 'Reasonable Compensation â Document the Basis',
          detail: `Officer compensation (${fmt(ownerComp)}) is ${_legacyRatio}% of total S-Corp compensation. The IRS applies a facts-and-circumstances test under Rev. Rul. 74-44 â there is no published safe-harbor ratio. S-Corp reasonable compensation is one of the most common examination triggers in IRS campaigns targeting pass-through entities.`,
  ...
  if (k1 > 5000 && estPay === 0) {
    findings.push({ level: 'high', icon: 'ð¨', title: 'No Estimated Tax Payments â Penalty Risk',
      detail: `With ${fmt(k1)} in K-1 income, you are likely required to make quarterly estimated payments. Failure to pay results in IRS underpayment penalties, charged at the federal short-term rate plus 3% (IRC Â§6621) and reset quarterly by the IRS (recently in the 6â8% range).`,
      action: `Estimated quarterly payment: approx. ${fmt(accurateQuarterly)}. Due dates: April 15, June 15, September 15, January 15.` })
  } else if (estPay > 0) {
    findings.push({ level: 'good', icon: 'â', title: 'Estimated Payments Recorded',
      detail: `${fmt(estPay)} in estimated payments on file. Next quarterly deadline: ${deadlines[month]}.`,
      action: null })
  }

  if (revenue > 50000 && dep === 0) {
    findings.push({ level: 'medium', icon: 'â ï¸', title: 'No Depreciation Recorded',
      detail: 'Businesses with equipment, vehicles, computers, or property can deduct depreciation â often reducing taxable income significantly.',
      action: `If you own any business assets, enter depreciation under Â§179 (full first-year deduction) or MACRS. A $20,000 asset could reduce your tax by ${fmt(Math.round(20000 * _marginalRate))}+ at your ${pct(_marginalRate * 100)} marginal rate.` })
  }

  if (k1 < 0 && isPassthroughEntity(b.entityType)) {
    const qbiLoss = Math.abs(k1)
    findings.push({
      level: 'info', icon: 'ð', title: 'QBI Loss Generated â Track Your Carryforward',
      detail: `Your ${fmt(qbiLoss)} business loss generates a Â§199A QBI loss carryforward. When your business returns to profitability, this carryforward will reduce (or eliminate) your Â§199A deduction base for that year â potentially increasing your future tax liability if not planned for.`,
      action: `Next year, enter ${fmt(qbiLoss)} (as a positive number) in the "Prior Year QBI Loss Carryforward" field in Step 2. This ensures your future QBI deduction is correctly calculated per IRC Â§199A(c)(2). Also confirm with your CPA that your S Corp stock and debt basis is sufficient to deduct the loss in the current year (Form 7203).`,
    })
  }
  ...
  if (isPassthroughEntity(b.entityType) && k1 > 10000) {
    ...
    const _limitPrefix = _limitApplied === 'wage' ? `Your deduction is currently reduced by ${fmt(_qbiGap)} due to the Â§199A(b)(2) wage/UBIA limit â increasing W-2 wages paid by the entity or qualified property (UBIA) â both reported on the K-1 Â§199A statement (Box 17 Code V) â could recapture it. `
                       ...
                       : _limitApplied === 'min400' ? `Your deduction is set to the Â§199A(i) OBBBA minimum of ${fmt(qbi)} â without this floor, your regular calc would have been lower. `
                       : ''
    const _aggNote = _agg && _aggDisc ? ` â  Aggregation assumed: ${_aggDisc}` : ''
    findings.push({ level: 'good', icon: 'â', title: `QBI Deduction Applied â ${fmt(qbi)} Deduction`,
      detail: `The Qualified Business Income deduction (IRC Â§199A) is applied to your K-1 income, reducing your taxable income by ${fmt(qbi)} â worth roughly ${fmt(Math.round(qbi * _marginalRate))} in federal tax at your ${(_marginalRate * 100).toFixed(0)}% marginal rate.`,
  ...
  if (isCCorpEntity(b.entityType) && revenue > 0) {
    findings.push({ level: 'medium', icon: 'ð¡', title: 'C-Corp Double Taxation',
      detail: 'C-Corp profits are taxed at 21% at the entity level. Dividends distributed to you are then taxed again at qualified dividend rates (0â20%) on your personal return.',
  ...
  if (rentalIncome > 0 || isREP) {
    if (isREP) {
      findings.push({ level: 'info', icon: 'ð ', title: 'Real Estate Professional â Criteria Checklist',
        detail: 'You have REP status selected. Under IRC Â§469(c)(7), you must meet ALL three tests each tax year to deduct rental losses without limitation:',
        action: `â  MORE THAN 750 HOURS in real property trades or businesses â of which MORE THAN 500 hours must be in activities where you materially participate.\n\nâ¡ MORE THAN 50% of your total personal service time across all work must be in real estate activities.\n\nâ¢ â ï¸ IMPORTANT: If you have a full-time W-2 job, qualifying as a REP is extremely difficult. The IRS scrutinizes this heavily. Document your time with contemporaneous daily logs. Without proper documentation, REP status will likely be disallowed on audit.` })
    } else {
      findings.push({ level: 'info', icon: 'ð ', title: 'Rental Income Detected â REP Status Could Unlock Full Deductions',
        detail: 'Rental losses are normally "passive" and can only offset other passive income. Qualifying as a Real Estate Professional makes your rental losses fully deductible against all income â including W-2 wages and business income.',
        action: `To qualify as a REP you must meet ALL of these each year:\n\nâ  More than 750 hours in real property trades or businesses â of which 500+ must be in activities where you materially participate.\n\nâ¡ More than 50% of your total working hours across ALL jobs must be in real estate.\n\nâ ï¸ If you have a full-time W-2 job, qualifying is very difficult. Most full-time W-2 earners cannot meet the 50% test. If you believe you qualify, check the REP box on the Tax Return page and maintain detailed daily time logs.` })
    }
  }

  if (isPastYear) {
    findings.push({ level: 'info', icon: 'ð', title: `${year} Quarterly Deadlines â All Past Due`,
      detail: `All four ${year} quarterly estimated tax deadlines have passed (Apr 15, Jun 15, Sep 15 ${year}, Jan 15 ${year + 1}). If you missed payments, the IRS underpayment penalty (IRC Â§6654) applies at the current federal short-term rate + 3%.`,
      action: estPay > 0
    ...
    findings.push({ level: 'info', icon: 'ð', title: `Next Quarterly Deadline: ${deadlines[month]}`,
  ...
  if (_suspendedLoss > 0) {
    ...
    findings.push({
      level: 'high',
      icon: 'ð¨',
      title: `S-Corp Loss Suspended â ${fmt(_suspendedLoss)} Not Deductible This Year (IRC Â§1366(d))`,
      detail: `Your S-Corp K-1 loss exceeds your combined stock and debt basis. Under IRC Â§1366(d)(1), the deductible loss is limited to adjusted stock basis plus indebtedness basis. The suspended ${fmt(_suspendedLoss)} is NOT included in your current-year tax calculation â it carries forward and becomes deductible when you restore sufficient basis.` +
        (_suspEntities.length > 0
          ? '\n\n' + _suspEntities.map(r =>
              `â¢ ${r.name || r.type}: gross loss ${fmt(Math.abs(r.k1Gross))} | allowed ${fmt(Math.abs(r.k1Allowed))} | suspended ${fmt(r.suspended)}` +
              (r.totalBasis != null ? ` (basis: ${fmt(r.totalBasis)})` : '')
            ).join('\n')
          : ''),
      action: `To restore basis and unlock suspended losses: (1) Make additional capital contributions â increases stock basis per IRC Â§1367(a)(1)(A). (2) Loan money personally to the S-Corp â creates debt basis per Â§1367(b)(2)(A); must be a bona fide loan with documentation. (3) In a profitable future year suspended losses release automatically. File Form 7203 each year you have an S-Corp loss or distribution. Discuss basis restoration strategy with your CPA.`,
    })
  }

  // C-10 FIX: S-Corp loss with no stock basis entered. The engine now conservatively
  // suspends a no-basis loss (Â§1366(d)), so a freshly-computed record lands in the
  ...
  if (_suspendedLoss === 0 && _sCorpLossNoBasis > 0) {
    findings.push({
      level: 'high',
      icon: 'ð¨',
      title: `Enter S-Corp Stock Basis â ${fmt(_sCorpLossNoBasis)} Loss Limited Until Basis Is Entered (IRC Â§1366(d))`,
      detail: `Your S-Corp K-1 shows a ${fmt(_sCorpLossNoBasis)} loss, but no beginning stock basis has been entered. Under IRC Â§1366(d)(1) your deductible loss is capped at your combined stock + debt basis, and any excess is suspended and carried forward (Â§1366(d)(2)). With nothing entered, the Tax Tracker conservatively assumes $0 basis and suspends the full loss â a higher, more conservative tax â until you provide your basis. (An estimate saved before this basis check may still show the loss as fully deducted; re-open and re-save it in the Tax Tracker to refresh.)`,
      action: `Open the S-Corp entity in Step 1 â "Stock & Debt Basis (Form 7203)" and enter your beginning-of-year stock basis (Form 7203, Line 1) plus any debt basis (Part II). With $0 basis the entire loss is suspended; with basis at or above the loss it is fully deductible this year. Your CPA tracks this figure on Form 7203 each year.`,
    })
  }
  ...
  if (isREP) {
    ...
    if (_nonREW2 > 75000) {
      findings.push({
        level: 'high',
        icon: 'ð¨',
        title: `REP Status With ${fmt(_nonREW2)} in Outside Employment â High Audit Risk`,
        detail: `You have Real Estate Professional status selected alongside ${fmt(_nonREW2)} in W-2 income from non-real-estate employment. IRC Â§469(c)(7)(B) requires MORE THAN 50% of ALL personal services during the year to be in real property trades or businesses where you materially participate. With significant outside employment, you would need to document more real estate hours than all other work combined. The IRS actively targets this combination â it is one of the most frequently challenged positions in the pass-through entity / real estate space.`,
        action: `If you qualify: (1) Maintain contemporaneous daily time logs for ALL activities â not a year-end reconstruction. Logs must show date, property, activity, and hours. (2) You must exceed 750 hours in real property trades/businesses AND >50% of total working time must be in real estate. (3) A Â§1.469-9(g) aggregation election can simplify material participation tracking across multiple properties. If you cannot document the 50% test, uncheck REP â rental losses become passive and offset only passive income. Discuss documentation requirements with your CPA.`,
      })
    }
  }
  ...
  if (_totEntRev > 10000 && _totEntExp > _totEntRev * 1.40) {
    ...
    findings.push({
      level: 'high',
      icon: 'ð¨',
      title: `Expenses Exceed Gross Receipts by ${_ratio - 100}% â Common IRS Scrutiny Pattern`,
  ...
  if (_totRevForVehicle > 15000 && _mileageDeduction === 0 && _vehicleExpenses === 0 && dep === 0) {
    ...
    findings.push({
      level: 'info',
      icon: 'ð',
      title: 'Vehicle / Mileage Deduction â Review Before Filing',
      detail: `No vehicle or mileage deduction is recorded. If you drive for business purposes â client visits, supply runs, business banking, attending professional education, or travel between job sites â you may be entitled to a vehicle deduction. The IRS standard mileage rate for ${year} is ${_stdMileCents}Â¢ per business mile. Personal commuting (home â regular office) is never deductible.`,
      action: `Two methods to choose from (must select one method per vehicle and generally stick with it):\n\nâ  Standard Mileage Rate â multiply business miles by the IRS rate (${_stdMileCents}Â¢/mile for ${year}). Simple. Requires a mileage log.\n\nâ¡ Actual Expense Method â deduct actual costs (gas, insurance, registration, repairs, depreciation) Ã business-use percentage. Can yield a larger deduction for high-cost or high-depreciation vehicles.\n\nRequirements: maintain a contemporaneous mileage log showing date, destination, business purpose, and miles for each trip. Apps like MileIQ, Everlance, or a simple spreadsheet work. The IRS requires contemporaneous records â reconstructed logs are regularly disallowed on audit.`,
    })
  }
    ...
    <div>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 11, color: '#64748B', margin: 0, lineHeight: 1.5 }}>
          These indicators reflect common patterns associated with IRS scrutiny â they are not a prediction of audit selection or probability. The IRS uses proprietary scoring and methods not publicly disclosed. Consult a licensed tax professional before making any filing decisions.
        </p>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {activeFindings.length === 0 && reviewedFindings.length > 0 && (
          <div style={{ fontSize: 13, color: SL, textAlign: 'center', padding: '12px 0', background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 12 }}>
            â All findings reviewed. See the reviewed list below to revisit any of them.
          </div>
        )}
      ...
      {reviewedFindings.length > 0 && (
        <div style={{ marginTop: 16 }}>
          ...
          >
            {showReviewed ? 'â¾' : 'â¸'} Reviewed ({reviewedFindings.length})
          </button>
          {showReviewed && (
...
// ââ TAB 2: Tax Optimization ââââââââââââââââââââââââââââââââââââââââââââââââââ
function TaxOptimization({ rec }) {
  ...
  if (!hasIncome) return (
    <div style={{ textAlign: 'center', padding: '48px 24px', background: '#F8FAFC', borderRadius: 14, border: '1px solid #E2E8F0' }}>
      ...
      <button onClick={() => window.location.href = '/calculate-tax'} style={{ padding: '10px 24px', background: B, border: 'none', borderRadius: 8, color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>Go to Calculator â</button>
    </div>
  )
  ...
  // Â§415(c) overall limits are year-specific â read from the centralized table, never
  ...
  if (isPassthrough) {
    if (isSCorpOwner && totalOfficerSalary === 0) {
      opportunities.push({
        icon: 'ð¦', title: 'SEP-IRA / Solo 401(k) â Set Officer Compensation First',
        priority: 'high', saving: null,
        detail: `S-Corp owners can only contribute to a SEP-IRA or Solo 401(k) based on their officer W-2 compensation â not K-1 distributions (IRC Â§402(h); IRS Pub. 560). With $0 officer compensation recorded, your current allowable contribution is $0.`,
        howTo: `Set reasonable officer compensation on Step 1 first. Once salary is recorded, you can contribute up to ${Math.round(SEP_IRA_RATE * 100)}% of that salary (max ${fmt(sepIraMax)} in ${year}). Example: a ${fmt(Math.round(k1 * 0.40))} salary would allow a ${fmt(Math.round(Math.min(sepIraMax, k1 * 0.40 * SEP_IRA_RATE)))} SEP-IRA contribution â saving approx. ${fmt(Math.round(Math.min(sepIraMax, k1 * 0.40 * SEP_IRA_RATE) * marginalRate))} in federal tax.`
      })
    } else if (maxSEP > 0) {
      const sepTaxSaved = Math.round(maxSEP * marginalRate)
      const sepDetail = isSCorpOwner
        ? `SEP-IRA contributions are employer-only, based on your W-2 officer compensation (not K-1 distributions â IRC Â§402(h)). At ${fmt(totalOfficerSalary)} officer compensation, the max SEP-IRA contribution is ${Math.round(SEP_IRA_RATE * 100)}% Ã ${fmt(totalOfficerSalary)} = ${fmt(maxSEP)} (max ${fmt(sepIraMax)}). The S-Corp makes the contribution at the entity level, deductible on Form 1120-S.`
      ...
      const sepDeadline = isSCorpOwner
        ? 'your S-Corp\'s tax return due date including extensions â typically September 15 for S-Corporations (Form 1120-S). Note: October 15 (the individual return extension) does NOT extend the S-Corp\'s SEP-IRA funding deadline.'
        : 'your individual return due date including extensions â typically October 15.'

      opportunities.push({
        icon: 'ð¦', title: 'SEP-IRA', priority: 'high',
        saving: sepTaxSaved,
        detail: sepDetail,
      ...
      if (isSCorpOwner) {
        ...
        opportunities.push({
          icon: 'ð°', title: 'Solo 401(k) â Higher Contribution than SEP-IRA',
          priority: 'high',
          saving: solo401kTaxSaved,
          detail: `Unlike a SEP-IRA (employer contributions only), a Solo 401(k) has two components that stack:\nâ¢ Employee elective deferral: up to ${fmt(solo401kDeferral)} (${year} limit, pre-tax or Roth â IRC Â§401(k))\nâ¢ Employer profit-sharing: 25% of your officer W-2 compensation = ${fmt(maxSolo401kEmployer)}\nâ¢ Combined total: ${fmt(maxSolo401k)} â vs. ${fmt(maxSEP)} under a SEP-IRA alone.\nBoth contributions flow from your S-Corp but are made separately.`,
          howTo: `At your ${pct(marginalRate * 100)} marginal rate, the max Solo 401(k) saves approx. ${fmt(solo401kTaxSaved)} â roughly ${fmt(solo401kTaxSaved - Math.round(maxSEP * marginalRate))} more than a SEP-IRA alone. Requires a plan document established before December 31 of the plan year. Employee deferral comes from your W-2 payroll (must be elected before December 31); employer contribution due by September 15 (S-Corp extension deadline). Available at Fidelity, Schwab, or Vanguard.`
        })
      }
    }
  }
  ...
  if (revenue > 30000 && dep === 0) {
    opportunities.push({
      icon: 'ðï¸', title: 'Â§179 Equipment Deduction', priority: 'medium',
      saving: null,
      detail: 'Â§179 lets you deduct the full cost of qualifying equipment, vehicles, and business property in the year of purchase (up to $2.5M in 2025 under the One Big Beautiful Bill Act; phase-out begins above $4M of qualifying purchases). Bonus depreciation is 100% for property placed in service after January 19, 2025.',
      howTo: 'If you purchased any computers, phones, furniture, vehicles, or equipment for the business this year â even partially â enter the cost under Depreciation. The deduction can be substantial.'
    })
  }

  const homeOfficeHowTo = isSCorpOwner
    ? 'The space must be used exclusively for business. Calculate your home office percentage (office sq ft Ã· total home sq ft) and apply to rent/mortgage interest, utilities, and insurance. For S-Corp owners, the correct method is an accountable plan reimbursement â the S-Corp pays you back for the business-use portion of home expenses, deducts the payment as a business expense, and the reimbursement is excluded from your W-2 income (IRC Â§62(a)(2)(A); Treas. Reg. Â§1.62-2). Do NOT use Schedule C â that form is for sole proprietors only. S-Corp shareholders who are also W-2 employees cannot deduct unreimbursed employee business expenses under current law (TCJA Â§67(g)).'
    : /partnership|mmllc/i.test(b.entityType || '')
    ? 'The space must be used exclusively for business. Calculate your home office percentage (office sq ft Ã· total home sq ft) and apply to rent/mortgage interest, utilities, and insurance. Partners may deduct unreimbursed partnership expenses (UPE) directly on Schedule E Part II, or the partnership can reimburse through an accountable plan â which is deductible at the entity level and excluded from the partner\'s income.'
    : 'The space must be used exclusively for business. Calculate your home office percentage (office sq ft Ã· total home sq ft) and apply to rent/mortgage interest, utilities, and insurance. Claim on Schedule C using Form 8829 (actual expense method) or the simplified method ($5/sq ft, up to 300 sq ft).'

  opportunities.push({
    icon: 'ð ', title: 'Home Office Deduction', priority: 'medium',
    saving: null,
  ...
  if (sCorpEntities.length > 0 && totalOfficerSalary > 0 && sCorpK1 > 50000) {
    ...
    // path â manual P&L folds officer salary into totalExpenses, and synced P&L is stored
    ...
    // (92.35% Â§1402(a)(12) factor + SS wage-base cap from the owner's FICA-subject wages),
    // so this figure matches the filed-return panel â critically, it no longer overstates
    ...
    if (seTaxSaved > 1000) {
      opportunities.push({
        icon: 'ð¼', title: 'S-Corp Salary vs. Distribution Split', priority: 'high',
        saving: seTaxSaved,
        detail: `Your S-Corp structure means your share of the business's ordinary income reported on the K-1 is not subject to self-employment tax â only your W-2 officer wages are. That treatment applies to the K-1 income whether or not you distribute it; it is not a benefit of taking distributions. FICA is 15.3% on wages up to the Social Security wage base (${fmt(getTable(year).ssWageBase)} for ${year}) and 2.9% Medicare-only above it. Versus operating as a sole proprietor â who owes SE tax on all net earnings â this saves roughly ${fmt(seTaxSaved)} on your ${fmt(sCorpK1)} of K-1 income (the profit remaining after your officer wages).`,
        howTo: `Estimated SE-tax savings vs. a sole proprietorship: ~${fmt(seTaxSaved)}. This figure already reflects the 92.35% net-earnings factor (IRC Â§1402(a)(12)) and the Social Security wage-base cap â because your W-2 wages count toward that ${fmt(getTable(year).ssWageBase)} cap, only the 2.9% Medicare portion applies to K-1 income once your wages use it up, so the savings is smaller for higher earners. The savings comes from the S-Corp structure, not from maximizing distributions â pay yourself reasonable W-2 compensation FIRST and keep documentation showing it is reasonable for your role. There is no IRS safe-harbor percentage; practitioners commonly use 35â45% of total officer compensation (salary Ã· (salary + distributions)) only as a rough starting point. Setting salary too low to enlarge the untaxed portion is the most common S-Corp audit trigger (Rev. Rul. 74-44).`
      })
    }
  }

  opportunities.push({
    icon: 'ð¥', title: 'Health Savings Account (HSA)', priority: 'medium',
  ...
  if (isSCorpEntity(b.entityType) && revenue > 0) {
    opportunities.push({
      icon: 'ð¡', title: 'Augusta Rule â IRC Â§280A(g)', priority: 'low',
      saving: null,
      ...
      howTo: 'Document business meetings, shareholder meetings, or strategy sessions held at your home. Pay fair market rent (research comparable event venue rates in your area). Keep written agreements. Maximum benefit: fair market rate Ã 14 days, deductible to the S-Corp.'
    })
  }
...
// ââ TAB 3: IRS Schedule Map ââââââââââââââââââââââââââââââââââââââââââââââââââ
// F21 FIX: Each form card now shows a coverage badge â "â Data entered" (green)
// or "â  Review needed" (amber). Coverage is derived from record data already
...
function IRSCompliance({ rec }) {
  ...
  // F21 FIX: coverage helpers â derived from the record's actual data
  ...
  schedules.push({ form: 'Form 1040', title: 'U.S. Individual Income Tax Return', status: 'required', covered: hasK1Data || hasW2Data || hasRevenue, detail: 'Your main personal tax return. All income sources flow here â W-2, K-1, Schedule E, Schedule C.', deadline: `April 15, ${year + 1}` })
  ...
  if (hasSCorpK1) {
    ...
    schedules.push({ form: 'Schedule K-1 (1120-S)', title: 'Shareholder Share of Income', status: 'required', covered: hasK1Data, detail: `Your ${fmt(sCorpK1Amount)} share of S-Corp ordinary business income (Box 1) flows to your personal return via this form. Your K-1 figures are reported on Schedule E, Part II â keep your K-1 as supporting documentation.`, deadline: `Issued with Form 1120-S` })
    schedules.push({ form: 'Schedule E (Part II)', title: 'Supplemental Income â S-Corp K-1', status: 'required', covered: hasK1Data, detail: 'Reports your K-1 income on your personal return. Passive vs. active participation rules apply.', deadline: 'Filed with Form 1040' })
  }
  if (hasPartnershipK1) {
  ...
  if (hasRealEstate) {
    ...
    if (realEstateAmount < 0) {
      schedules.push({ form: 'Form 8582', title: 'Passive Activity Loss Limitations', status: 'required', covered: hasK1Data, detail: 'Computes the allowed and suspended portions of passive rental losses (IRC Â§469).', deadline: 'Filed with Form 1040' })
    }
  }
  ...
  if (isPassthroughEntity(entity) && k1 > 0) {
    ...
    const _lossNote = (_useForm8995A && (_currentYearQbiLoss || _priorQbiLoss)) ? ' QBI loss detected â see Form 8995-A Schedule C for loss netting.' : ''
    const _coopNote = (_isCoopPatron && _useForm8995A) ? ' Co-op patron status flagged â see Form 8995-A Schedule D.' : ''
    const _aggNote = _agg ? ' â  QBI aggregation across entities assumed (Reg. Â§1.199A-4 election). Confirm formal election on this form.' : ''
  ...
  schedules.push({
    ...
    detail: isSCorpEntity(entity)
      ? 'Carries Additional Medicare Tax (0.9%, Form 8959) and Net Investment Income Tax (3.8%, Form 8960) to Form 1040 Line 17. Note: SE tax does NOT apply to S-Corp K-1 income â IRC Â§1402(a)(2).'
  ...
  if (_capGainTotal !== 0) {
    ...
    schedules.push({ form: 'Form 8949', title: 'Sales and Other Dispositions of Capital Assets', status: 'required', covered: hasCapGains, detail: 'Lists individual capital asset sales â purchase date, sale date, basis, proceeds. Subtotals roll up to Schedule D.', deadline: 'Filed with Schedule D' })
  }

  const _form4797 = nf(f.form4797 ) || 0
  if (_form4797 !== 0 || _unrec1250 > 0) {
    schedules.push({ form: 'Form 4797', title: 'Sales of Business Property', status: 'required', covered: hasForm4797 || _unrec1250 > 0, detail: `Reports ${_form4797 !== 0 ? 'ordinary gain/loss on Â§1231 property and Â§1245/Â§1250 recapture' : 'unrecaptured Â§1250 gain (depreciation recapture on real property, taxed at max 25%)'}.`, deadline: 'Filed with Form 1040' })
  }
    ...
    <div>
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: coveredCount === totalCount ? '#F0FDF4' : '#FFFBEB', border: '1px solid ' + (coveredCount === totalCount ? '#86EFAC' : '#FDE68A'), borderRadius: 8, padding: '7px 14px', fontSize: 12, fontWeight: 600, color: coveredCount === totalCount ? '#166534' : '#78350F' }}>
          {coveredCount === totalCount ? 'â' : 'â '} {coveredCount} of {totalCount} required schedules have data entered{coveredCount < totalCount ? ` â ${totalCount - coveredCount} require action` : ' â all covered'}
        </div>
      </div>
          ...
          <div key={i} style={{ background: '#fff', border: '1px solid ' + (s.covered ? '#E2E8F0' : '#FDE68A'), borderRadius: 10, padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 10, fontWeight: 700, background: s.covered ? '#F0FDF4' : '#FFFBEB', color: s.covered ? '#166534' : '#78350F', border: '1px solid ' + (s.covered ? '#86EFAC' : '#FDE68A'), borderRadius: 4, padding: '1px 7px' }}>
                {s.covered ? 'â Data entered' : 'â  Review needed'}
              </span>
            </div>
            ...
            <div style={{ fontSize: 11, color: '#64748B' }}>ð {s.deadline}</div>
          </div>
        ))}
      </div>
      <div style={{ background: N, borderRadius: 12, padding: '20px 24px' }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: '#93C5FD', letterSpacing: '1px', marginBottom: 14 }}>KEY DEADLINES â TAX YEAR {year}</div>
          ...
          {upcomingDeadlines.map((d, i) => (
            <div key={i} style={{ display: 'flex', gap: 16, alignItems: 'center', opacity: d.isPast ? 0.4 : 1 }}>
              ...
              <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)', textDecoration: d.isPast ? 'line-through' : 'none' }}>
                {d.event}{!d.isPast && d.daysAway >= 0 && d.daysAway <= 60 ? ` â ${d.daysAway} days` : ''}
              </span>
            </div>
          ))}
        </div>
...
// ââ TAB 4: Reports & Tools âââââââââââââââââââââââââââââââââââââââââââââââââââ
      ...
      <div style={{ padding: '28px 32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: SL, marginTop: 4 }}>Generated {now} Â· TaxStat360</div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => window.print()} style={{ padding: '8px 18px', background: B, color: '#fff', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>ð¨ Print / Save PDF</button>
            <button onClick={onClose} style={{ padding: '8px 14px', background: '#F1F5F9', color: SL, border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>â Close</button>
          </div>
        </div>
        {rec && (
          <div style={{ background: '#F8FAFC', borderRadius: 10, padding: '16px 20px', marginBottom: 16, border: '1px solid #E2E8F0' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: SL, letterSpacing: '1px', marginBottom: 12 }}>LAST SAVED CALCULATION{rec.savedAt ? ` â ${rec.savedAt}` : ''}</div>
            {[
        ...
        <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 10, padding: '14px 20px', marginBottom: 16 }}>
          <div style={{ fontWeight: 700, color: '#1D4ED8', fontSize: 13, marginBottom: 10 }}>ð IRS Schedule Mapping</div>
          {[
            ['Schedule E (Part II)', 'K-1 income from S-Corps, Partnerships, and Trusts'],
            ['Form 8995', 'QBI deduction â up to 20% of qualified business income. Full deduction below $197,300 single / $394,600 MFJ (2025); W-2 wage / UBIA limit applies above (IRC Â§199A(b)(2)).'],
            ['Form 8959', 'Additional Medicare Tax â 0.9% on wages over $200K'],
            ['Schedule A', 'Itemized deductions â mortgage, taxes, charitable'],
...
function BriefingModal({ onClose, rec }) {
  ...
  // briefing uses the same deduction the engine does â not always the standard deduction.
  ...
  // entities, after Â§164(f) adjustments) while the prior inline calc used raw k1Income,
  ...
  if (isSCorpEntity(b.entityType) && officerSal > 0 && k1 > 0) {
    ...
    points.push(`Officer compensation is ${fmt(officerSal)} against ${fmt(k1)} of K-1 distributions â a ${pct(ratio * 100)} salary-to-total-compensation ratio. Practitioners commonly target 35â45% (Watson v. Commissioner, 668 F.3d 1008 (8th Cir. 2012)); the IRS applies a facts-and-circumstances test with no published safe harbor. ${ratio < SCORP_REASONABLE_COMP_RATIO_THRESHOLD ? 'Review whether the salary adequately reflects the services rendered.' : 'Document the basis for the salary level â role, hours, and comparable pay.'}`)
  } else if (isSCorpEntity(b.entityType) && officerSal === 0 && k1 > 0) {
    points.push(`This S-Corp shows ${fmt(k1)} of K-1 income but no officer W-2 compensation on file. Shareholder-employees performing services must take reasonable W-2 compensation (Rev. Rul. 74-44) â determine an appropriate salary and ensure FICA is withheld.`)
  }
  if (isScheduleCType(b.entityType) || /partner/i.test(b.entityType || '')) {
    points.push(`Net earnings from this ${isScheduleCType(b.entityType) ? 'sole proprietorship / SMLLC' : 'partnership / LLC'} are subject to self-employment tax (IRC Â§1401) in addition to income tax. One-half of SE tax is deductible above the line (Â§164(f)).`)
  }
  if (qbi > 0) {
    points.push(`A Â§199A QBI deduction of approximately ${fmt(qbi)} is reflected â worth roughly ${fmt(Math.round(qbi * marginalRate))} at the ${pct(marginalRate * 100)} marginal rate. Above the ${year} income threshold the W-2 wage / UBIA limit (Â§199A(b)(2)) can reduce it; confirm entity W-2 wages and qualified property.`)
  } else if (isPassthroughEntity(b.entityType) && k1 > 0) {
    points.push(`Review Â§199A QBI eligibility for the pass-through income, including SSTB status and the W-2 wage / UBIA limitations that apply above the ${year} income threshold.`)
  }
  if (totalIncome > 0) {
    points.push(estPay > 0
      ? `${fmt(estPay)} in estimated payments is on file. Confirm the remaining quarterly installments meet a Â§6654 safe harbor (90% of current-year tax, or 100%/110% of prior-year tax).`
      : `No estimated payments are on file. With ${fmt(totalIncome)} of income, quarterly estimates are likely required â approximately ${fmt(quarterly)} per quarter (due Apr 15, Jun 15, Sep 15, Jan 15) â to avoid Â§6654 underpayment penalties.`)
  }
  const gather = []
  if (!(recDepreciation(rec) > 0)) gather.push('depreciation / Â§179 on business assets')
  ...
  const sign = (v) => (v < 0 ? 'â' + fmt(Math.abs(v)) : fmt(v))
  ...
  const plain = [
    'CPA PLANNING BRIEFING â TaxStat360',
    `Business: ${displayName}${bizEin ? ' Â· EIN: ' + bizEin : ''}${bizAddress ? ' Â· ' + bizAddress : ''}`,
    `Prepared ${now} Â· Tax year ${year} Â· ${filingLabel}`,
    'Planning summary for discussion â not a tax return, not for filing.',
    '',
    'ENTITY STRUCTURE',
    ...(entities.length
    ...
    ...(qbi > 0 ? [`  Â§199A QBI deduction (est.): -${fmt(qbi)}`] : []),
    ...
    '  - Simplified estimate: does NOT separately model Â§461(l) EBL, NIIT (3.8%), Additional Medicare Tax (0.9%), or AMT.',
    ...(rec.totalTax ? [`  - Tax Tracker full estimate (all applicable federal taxes): ${fmt(rec.totalTax)}.`] : []),
    '',
    'Figures are estimates for planning discussion only â not professional tax advice and not for filing.',
  ].join('\n')
      ...
      <div style={{ padding: '28px 32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
          <div>
            ...
            {bizEin && <div style={{ fontSize: 12, color: SL, marginTop: 2 }}>EIN: {bizEin}</div>}
            {bizAddress && <div style={{ fontSize: 12, color: SL }}>{bizAddress}</div>}
            <div style={{ fontSize: 13, color: SL, marginTop: 4 }}>Prepared {now} Â· Tax year {year} Â· {filingLabel} Â· TaxStat360</div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={handleCopy} style={{ padding: '8px 16px', background: copied ? G : B, color: '#fff', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>{copied ? 'â Copied' : 'ð Copy'}</button>
            <button onClick={() => window.print()} style={{ padding: '8px 16px', background: '#F1F5F9', color: SL, border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>ð¨ Print</button>
            <button onClick={onClose} style={{ padding: '8px 14px', background: '#F1F5F9', color: SL, border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>â</button>
          </div>
        </div>

        <DismissibleNotice storageKey="tx360.cpaBriefingBanner.dismissed">
          This briefing is auto-generated from the figures you entered. It is an estimate to guide a planning conversation â not a tax return, not tax advice, and not for filing. Verify with your CPA.
        </DismissibleNotice>
        ...
        <div style={{ background: '#F8FAFC', borderRadius: 10, padding: '16px 20px', margin: '14px 0', border: '1px solid #E2E8F0' }}>
          ...
          {entities.length ? entities.map((e, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #F1F5F9', fontSize: 13 }}>
              <span style={cellL}>{e.type || 'Entity'} Â· {ownPct(e.own)}%</span>
              <span style={cellR}>net {fmt(getEntityNetProfit(e))}{num(e?.pnl?.officerSalary) > 0 ? ` Â· salary ${fmt(num(e.pnl.officerSalary))}` : ''}</span>
            </div>
          )) : (
        ...
        <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 10, padding: '16px 20px', marginBottom: 14 }}>
          ...
          {[
            ...
            ...(qbi > 0 ? [['Â§199A QBI deduction (est.)', sign(-qbi)]] : []),
            ['Taxable income (est.)', fmt(taxable)],
            ['Estimated federal income tax', fmt(fedTax)],
            ...(seTax > 0 ? [['Estimated self-employment tax', fmt(seTax)], ['Estimated total federal tax', fmt(totalFedTax)]] : []),
            ['Estimated effective rate', pct(effectiveRate * 100)],
        ...
        <div style={{ fontSize: 11, color: SL, textAlign: 'center', lineHeight: 1.6 }}>
          Auto-generated from your TaxStat360 data Â· Planning estimate only â not professional tax advice and not for filing. Consult a licensed tax professional before filing.
        </div>
      </div>
    </Modal>
  )
...
// (1) All monetary display now uses the shared fmt() from utils/formatMoney â
...
//     "Scenario total: $X  â  vs. your current estimate: $Y  â  Diff: $Z"
...
function SimulatorModal({ onClose, rec }) {
  ...
  // F15 FIX: chg() now uses shared fmt() â no local simFmt
  const chg = (baseVal, scenVal) => {
    const diff = scenVal - baseVal
    if (diff === 0) return null
    return <span style={{fontSize:11,fontWeight:700,color:diff>0?'#DC2626':'#059669',marginLeft:6}}>{diff>0?'â+'+fmt(diff):'â'+fmt(Math.abs(diff))}</span>
  }
  ...
  const presets = [
    { id:'adv15',   icon:'ð¢', label:'$15K Advertising',    color:'#D97706' },
    { id:'adv30',   icon:'ð£', label:'$30K Advertising',    color:'#D97706' },
    { id:'equip20', icon:'ð§', label:'$20K Equipment',       color:'#2563EB' },
    { id:'equip50', icon:'ðï¸', label:'$50K Equipment',       color:'#7C3AED' },
    { id:'sep',     icon:'ð¦', label:'Max SEP-IRA',          color:'#059669' },
    { id:'revenue', icon:'ð', label:'+$50K Gross Receipts',        color:'#0891B2' },
    { id:'salary',  icon:'ð¼', label:'+$20K Salary',         color:'#475569' },
    { id:'custom',  icon:'âï¸', label:'Custom',               color:'#64748B' },
  ]

  // F15 FIX: row() uses shared fmt() throughout â not simFmt
      ...
      <div style={{padding:'24px 28px',fontFamily:'Inter,sans-serif'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:20}}>
          <div>
            ...
            <div style={{fontSize:12,color:'#64748B'}}>{entity} Â· Tax Year {taxYear} Â· {filing.toUpperCase()} Â· Changes don't affect your saved record</div>
          </div>
          <div style={{display:'flex',gap:8}}>
            ...
            <button onClick={onClose} style={{padding:'7px 13px',background:'#F1F5F9',color:'#64748B',border:'none',borderRadius:8,fontWeight:600,fontSize:13,cursor:'pointer'}}>â</button>
          </div>
        </div>
        <div style={{marginBottom:16}}>
          ...
          <>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
              <div style={{background:'#fff',border:'1px solid #E2E8F0',borderRadius:12,padding:'16px 18px'}}>
                <div style={{fontSize:11,fontWeight:700,color:'#64748B',letterSpacing:'0.5px',marginBottom:10}}>{entity.toUpperCase()} â ENTITY LEVEL</div>
            ...
            {/* F15 FIX: reconciliation line â scenario vs Step 2 full estimate */}
            {step2Estimate > 0 && (
              <div style={{background:'#F8FAFC',border:'1px solid #E2E8F0',borderRadius:8,padding:'10px 16px',marginBottom:8,fontSize:12,color:SL,display:'flex',gap:16,flexWrap:'wrap',alignItems:'center'}}>
                ...
                <span style={{color:'#CBD5E1'}}>â</span>
                <span>Difference: <strong style={{color: scenario.fedTax < step2Estimate ? '#059669' : '#DC2626'}}>{scenario.fedTax < step2Estimate ? 'â' : '+'}{fmt(Math.abs(Math.round(scenario.fedTax - step2Estimate)))}</strong></span>
                <span style={{fontSize:10,color:'#64748B'}}>(includes income tax, SE tax, NIIT, AMT â matches Step 2)</span>
              </div>
            )}
          </>
        )}
        ...
        <div style={{fontSize:11,color:'#64748B',textAlign:'center',marginTop:8}}>
          Uses {taxYear} federal brackets Â· {filing.toUpperCase()} Â· {fmt(stdDed)} std deduction Â· Does not include state tax, FICA, or AMT Â· Consult a licensed tax professional before implementing.
        </div>
      </div>
    </Modal>
  )
...
function NarrativeModal({ onClose }) {
  ...
  const [copied, setCopied] = useState(false)
  const narratives = [
    { title: 'Real Estate Professional Status', tag: 'REP Â· IRC Â§469(c)(7)', color: P, text: `Dear IRS Representative,\n\nThis letter responds to your inquiry regarding the taxpayer's Real Estate Professional (REP) classification under IRC Section 469(c)(7) for tax year 2025.\n\nThe taxpayer qualifies as a Real Estate Professional based on the following:\n\n1. MORE THAN 50% OF PERSONAL SERVICES\nThe taxpayer performed more than 50% of all personal services in real property trades or businesses in which they materially participated.\n\n2. MORE THAN 750 HOURS\nThe taxpayer performed more than 750 hours of services during the year satisfying the statutory threshold under IRC Â§469(c)(7)(B).\n\n3. MATERIAL PARTICIPATION\nThe taxpayer materially participated in each rental activity meeting the 500-hour test under Treas. Reg. Â§1.469-5T(a)(1).\n\nAs a result, rental real estate losses are treated as non-passive and are fully deductible pursuant to IRC Â§469(c)(7)(A).\n\nRespectfully submitted,` },
    { title: 'S-Corp Reasonable Compensation', tag: 'Officer Compensation Â· Rev. Rul. 74-44', color: R, text: `Dear IRS Representative,\n\nThis letter addresses your inquiry regarding officer compensation paid through the taxpayer's S-Corporation for tax year 2025.\n\nThe officer compensation represents reasonable compensation based on:\n\n1. INDUSTRY BENCHMARKS â Compensation was determined by reference to comparable salaries consistent with Rev. Rul. 74-44.\n\n2. DUTIES AND RESPONSIBILITIES â The officer-shareholder performs substantial services including business development, client management, and financial oversight.\n\n3. CORPORATE PROFITABILITY â The compensation represents a reasonable percentage of gross receipts consistent with industry norms.\n\nThe S-Corporation maintains complete payroll records and W-2 forms.\n\nRespectfully submitted,` },
    { title: 'K-1 Loss Deductibility', tag: 'Schedule E Â· IRC Â§1366(d)', color: '#0891b2', text: `Dear IRS Representative,\n\nThis letter responds to your inquiry regarding Schedule E losses reported from the taxpayer's S-Corporation K-1 for tax year 2025.\n\nThe K-1 losses are fully deductible for the following reasons:\n\n1. SHAREHOLDER BASIS â The taxpayer maintains sufficient stock basis under IRC Â§1366(d). Form 7203 is attached.\n\n2. AT-RISK RULES â The taxpayer is at risk for the full amount of the loss under IRC Â§465.\n\n3. MATERIAL PARTICIPATION â The taxpayer satisfies material participation standards under Treas. Reg. Â§1.469-5T.\n\nComplete corporate returns (Form 1120-S) and K-1 schedules are available upon request.\n\nRespectfully submitted,` },
  ]
      ...
      <div style={{ padding: '28px 32px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: SL, marginTop: 4 }}>Review and edit before sending â not a substitute for legal advice</div>
          </div>
          <button onClick={onClose} style={{ padding: '8px 14px', background: '#F1F5F9', color: SL, border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>â Close</button>
        </div>
        ...
        <div style={{ display: 'inline-block', background: '#F5F3FF', border: '1px solid #DDD6FE', borderRadius: 20, padding: '4px 14px', fontSize: 11, fontWeight: 700, color: P, marginBottom: 14 }}>ð {current.tag}</div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
          <button onClick={handleCopy} style={{ flex: 1, padding: '11px', background: copied ? G : P, color: '#fff', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>{copied ? 'â Copied!' : 'ð Copy to Clipboard'}</button>
          <button onClick={() => window.print()} style={{ flex: 1, padding: '11px', background: '#F1F5F9', color: SL, border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>ð¨ Print</button>
        </div>
        <div style={{ fontSize: 11, color: SL, textAlign: 'center' }}>â ï¸ Templates only â review with a licensed tax attorney before submitting to the IRS.</div>
      </div>
    </Modal>
  )
}

// ââ CODE CONSISTENCY AUDIT (June 2026) âââââââââââââââââââââââââââââââââââââââââ
...
//   (multi-entity aggregate with Â§164(f) adjustments); briefing was using k1Income
//   (raw K-1 total), which diverges on multi-entity returns. rec.seTax is now
//   written by TaxReturn.jsx buildRecord() at save time.
//
// ââ UX PASS (June 2026) ââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// - score 50â79: button enabled, pre-generation checklist shown (â / â  per field)
// - score â¥ 80: button enabled, no warning
function ReportsTab({ rec, onReport, onSimulator, onNarrative, onBriefing }) {
  ...
  // F20 FIX: pre-generation checklist items â positives and negatives
  ...
  // C-24: labels of the checklist items still missing â shown verbatim in the confirm gate.
  ...
  const tools = [
    {
      ...
      // C-24: in the 50â79% band, generating requires confirming the listed gaps.
      needsConfirm: score >= 50 && score < 80 && missingChecklist.length > 0,
    },
    { icon: 'ð¯', title: 'What-If Tax Simulator', desc: 'Model a financial decision before making it. Try different salary levels, add a deduction, or max a retirement account â see the estimated dollar impact on your projected tax.', btn: 'Open Simulator', color: N, action: onSimulator, available: true },
    { icon: 'ð', title: 'CPA Briefing', desc: 'An auto-generated planning summary of your tax position â entity structure, estimated federal liability, QBI, reasonable-comp and SE-tax notes, and quarterly estimates â organized as discussion points for your CPA. A planning summary, not a tax return; not for filing.', btn: 'Generate Briefing', color: N, action: onBriefing, available: isEnterprise(), requiredPlan: 'enterprise' },
    { icon: 'ð¡ï¸', title: 'Position Documentation', desc: 'Generates a written summary of the positions taken on your return with supporting documentation references. Useful for your CPA, your records, or as starting material for a professional response. Not a substitute for representation by a CPA, EA, or tax attorney.', btn: 'View Templates', color: N, action: onNarrative, available: isEnterprise(), requiredPlan: 'enterprise' },
  ]
          ...
          const card = (
            <div key={t.title} style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 14, padding: '24px', display: 'flex', gap: 20, alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                {/* F20 FIX: pre-generation checklist (50â79%) */}
                {t.checklist && (
                ...
                {t.needsConfirm && confirmingExport && (
                  <div role="alert" style={{ marginTop: 10, background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8, padding: '12px 14px', fontSize: 12, color: '#78350F' }}>
                    <div style={{ fontWeight: 700, marginBottom: 4 }}>â  This pack is incomplete</div>
...
export default function AIAnalysis() {
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#F0F4FF', fontFamily: 'Inter, system-ui, sans-serif', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <div style={{ background: '#fff', borderRadius: 16, border: '1.5px dashed #cbd5e1', padding: '48px 36px', maxWidth: 480, textAlign: 'center', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
          ...
          <h2 style={{ fontSize: 22, fontWeight: 800, color: '#0D1B3E', margin: '0 0 10px' }}>AI Analysis & Reporting â Professional Feature</h2>
          ...
          <div style={{ background: '#f8fafc', borderRadius: 10, padding: '16px', marginBottom: 24, textAlign: 'left' }}>
            {['ð¨ Officer compensation & audit risk flags', 'ð¡ Tax-saving strategy finder', 'ð IRS filing schedule map', 'ð One-click CPA export pack'].map(f => (
  ...
  const tabs = [
    { id: 'risk',       label: `ð ${FEATURE_AUDIT_RISK_SCAN}` },
    { id: 'optimize',   label: `ð¡ ${FEATURE_WHATIF_SIMULATOR}` },
    { id: 'compliance', label: 'ð IRS Schedule Map' },
    { id: 'reports',    label: 'ð Reports & Tools' },
  ]
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #E2E8F0', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
          ...
          <nav style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
            {[
              { label: 'Dashboard',    mobileLabel: 'â', path: '/dashboard' },
              { label: 'Tax Tracker',  mobileLabel: 'ð§®', path: '/calculate-tax' },
              { label: 'AI Analysis & Reporting', mobileLabel: 'ð¤', path: '/ai-analysis' },
              { label: 'Settings',     mobileLabel: 'â', path: '/settings' },
            ].map(link => (
          ...
          <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, padding: '16px 20px', textAlign: 'center', flexShrink: 0 }}>
            ...
            {score === 0 ? (
              <div style={{ fontSize: 11, color: SL, marginTop: 8, maxWidth: 160, lineHeight: 1.5 }}>
                ...
                to unlock all analysis â takes about 5 min
              </div>
            ) : missing.length > 0 ? (
