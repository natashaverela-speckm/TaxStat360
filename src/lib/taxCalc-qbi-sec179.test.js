// RENAMED (M6, audit F-07) from taxCalc-qbi179.test.js. Distinct from
// taxCalc-sec179.test.js (the §179(b)(3) LIMITATION function): THIS file covers
// the ENGINE's §199A/AGI netting of separately-stated §179 (box11_12).
...
describe('calcTaxReturn — §199A QBI basis nets §179 (box11_12)', () => {
  it('SPEC: §199A / Reg. §1.199A-3(b) — QBI reduced by separately-stated §179 (the reported bug)', () => {
  ...
  it('CHAR: inert when no §179 present (QBI on the full net)', () => {
    const input = { ...baseInput, k1Total: 100000, entities: [sCorp({ box11_12: '' })] }
    const r = calcTaxReturn(input)
    expect(r.qbi).toBe(20000)            // 20% of the full $100,000 net
  })

  it('CHAR: §179-netted QBI stays consistent with the AGI net (no double-count)', () => {
    const input = { ...baseInput, k1Total: 69500, entities: [sCorp({ box11_12: '30500' })] }
    const r = calcTaxReturn(input)
