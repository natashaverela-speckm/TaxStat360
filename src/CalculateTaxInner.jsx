...
// nf() (numeric coercion) is imported from utils/parseMoney.js â single shared definition (audit C-2).
 ...
 * labeled a directly-held Schedule E rental AND a Schedule C sole prop as "K-1" â neither
...
export function entityResultLabel(type) {
  ...
  if (isRealEstateEntity(type)) return 'Net (Sch. E)' // directly-held rental, Part I â no K-1
  if (isScheduleCType(type)) return 'Net (Sch. C)'    // sole prop / SMLLC â no K-1
...
// âââ ReasonableCompIndicator âââââââââââââââââââââââââââââââââââââââââââââââââ
...
function ReasonableCompIndicator({ officerSal, netProfit, grossRevenue, isSCorp }) {
  ...
  // F-02: Watson revenue-ratio advisory â independent of total-comp ratio
  ...
  // C-8 FIX: in a loss year the 35-45% gross-receipts heuristic is unreliable â the
  ...
  if (officerSal === 0) {
    return (
      <div style={{ background: '#FEF2F2', border: '1.5px solid #FECACA', borderRadius: 10, padding: '12px 14px', marginTop: 10, fontSize: 13 }}>
        <div role="alert" style={{ fontWeight: 700, color: R, marginBottom: 4 }}>ð¨ No Officer Compensation Set</div>
        <div style={{ color: '#7F1D1D', lineHeight: 1.6 }}>
          ...
          {isLossYear
            ? <> In a loss year, reasonable compensation is determined primarily by the <em>value of services you rendered</em> â not by gross receipts or net income. Discuss the appropriate salary with your CPA, who can evaluate comparable market pay for your role.</>
            : <> A common starting point here is about <strong>{fmt(minTarget)}/yr</strong> (â35â45% of your total take from the business). Note: this is a rough heuristic, not a statutory floor â the correct amount reflects the value of services you personally performed.</>
          }
        </div>
        <CompSources color="#7F1D1D">
          Paying $0 salary while taking distributions is the most common S-Corp audit trigger â the IRS
          can reclassify distributions as wages and assess back FICA plus penalties. Authority:
          Rev. Rul. 74-44; Treas. Reg. Â§1.162-7. The 35â45% range is a practitioner heuristic, not a
          statutory safe harbor â reasonable comp ultimately reflects the value of services you provide.
        </CompSources>
      </div>
    )
  }
  ...
  if (ratio < SCORP_REASONABLE_COMP_RATIO_THRESHOLD) {
    return (
      <div style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A', borderRadius: 10, padding: '12px 14px', marginTop: 10, fontSize: 13 }}>
        <div role="alert" style={{ fontWeight: 700, color: '#78350F', marginBottom: 4 }}>â  Officer Compensation May Be Too Low</div>
        <div style={{ color: '#78350F', lineHeight: 1.6 }}>
          ...
          a common target is 35â45%. Paying a reasonable salary first, then taking the rest as
          ...
          {watsonWarning && (
            ...
            ({fmt(officerSal)} Ã· {fmt(grossRevenue)}) â under the ~30% many advisors watch for in
        ...
        <CompSources color="#78350F">
          ...
          668 F.3d 1008 (8th Cir. 2012); Treas. Reg. Â§1.162-7; Rev. Rul. 74-44. The 35â45% range is a
          practitioner heuristic, not a safe harbor â reasonable comp depends on the value of services
          you provide.
        </CompSources>
      </div>
    )
  ...
  return (
    <div style={{ background: '#F0FDF4', border: '1.5px solid #86EFAC', borderRadius: 10, padding: '12px 14px', marginTop: 10, fontSize: 13 }}>
      <div style={{ fontWeight: 700, color: '#166534', marginBottom: 4 }}>â Officer Compensation Looks Reasonable</div>
      <div style={{ color: '#166534', lineHeight: 1.6 }}>
        Your salary is {(ratio * 100).toFixed(0)}% of total officer compensation â within the commonly
        recommended 35â45% range. Make sure FICA payroll taxes are withheld and remitted (quarterly
        Form 941).
        {watsonWarning && (
          <> One thing to watch: your salary is <strong>{(revRatio * 100).toFixed(0)}%</strong> of gross
          receipts ({fmt(officerSal)} Ã· {fmt(grossRevenue)}) â under the ~30% some advisors flag even when
          the total-comp ratio looks fine.</>
        )}
      </div>
      {watsonWarning && (
        <CompSources color="#166534">
          Treas. Reg. Â§1.162-7; Watson v. Commissioner, 668 F.3d 1008 (8th Cir. 2012). Salary-to-receipts
...
// âââ Integration tile âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {isConnected && hasFailed ? (
          ...
          <div style={{ fontSize: 11, color: integ.color, fontWeight: 600 }}>
            ...
            {syncedLabel && (
              <span style={{ color: SL, fontWeight: 400, marginLeft: 6 }}>Â· Synced {syncedLabel}</span>
            )}
          </div>
        ) : hasFailed ? (
          <div style={{ fontSize: 11, color: R, fontWeight: 600 }}>Connection failed â try again</div>
        ) : (
          <div style={{ fontSize: 11, color: '#64748B', fontWeight: 600 }}>Click to connect â</div>
        )}
...
// âââ Name record modal ââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: '28px 28px', maxWidth: 440, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        ...
        <p style={{ fontSize: 13, color: SL, margin: '0 0 16px', lineHeight: 1.6 }}>
          Give this record a descriptive name so you can find it later â or skip and we'll use the date.
        </p>
        <input
          type="text"
          value={name}
...
// âââ Manual entry panel âââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
export function ManualEntryPanel({ entity, onUpdate, onCancel, idx }) {
  ...
  // F2 FIX (UX audit â Critical): the manual P&L used to commit to the parent
  // entity ONLY when the user clicked "Save P&L â". If they instead collapsed the
  // panel (the "Edit P&L" toggle), collapsed the entity card, or advanced to Step 2,
  // every figure they typed was silently discarded â so the entity reached Step 2/3
  ...
  // button (relabeled "Done" in the F9 save-button consolidation; formerly "Save P&L â")
    ...
    <div style={{ background: '#F8FAFC', borderRadius: 10, padding: '16px 18px', marginTop: 10, border: '1px solid #E2E8F0' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div>
          <label style={lbl}>
            ...
            <InfoTip text={isRE ? 'Total gross rents received from this rental property before any expenses (Schedule E, line 3).' : 'Total gross receipts before any deductions â everything the business took in, before any expenses. For S-Corps and partnerships, enter the entity\'s gross receipts (your taxable share flows via K-1, not the full gross receipts amount). For Schedule C filers, enter Line 1 gross receipts, not Line 3 gross profit. Do NOT net out officer compensation â enter that separately below.'} />
          </label>
        ...
        <div>
          <label style={lbl}>
            ...
            <InfoTip text={isRE ? 'Recurring rental expenses: repairs, maintenance, property management, insurance, property tax, utilities, HOA dues, etc. (Schedule E). Exclude depreciation and advertising â those have their own fields below.' : 'Recurring business expenses: rent, utilities, software, insurance, professional fees, payroll (non-owner), etc. Exclude officer compensation, depreciation, and advertising â those have their own fields below.'} />
          </label>
        ...
        <div>
          <label style={lbl}>
            {/* TERMINOLOGY FIX 3.1: "Sec. 179 + MACRS + Bonus" used mixed notation. Every other IRC
                section in the app uses the Â§ symbol (Â§199A, Â§469, Â§1366, etc.). "Bonus" is informal
                for Â§168(k) bonus depreciation. Normalized to Â§ style throughout. */}
            Depreciation (Â§179 + MACRS + Â§168(k) Bonus)
            <InfoTip text={'Â§179 first-year expensing, MACRS (Modified Accelerated Cost Recovery System) regular depreciation, and Â§168(k) bonus depreciation on qualified business assets.\n\nEnter the total deductible depreciation for this entity this year.\n\nDo NOT include depreciation on personal-use assets.\n\nFor vehicles: use either the standard mileage rate OR actual expenses (including depreciation) â you cannot use both methods for the same vehicle.\n\nEnter the depreciation you (or your accountant) already computed â TaxStat360 uses this figure as entered and does not calculate bonus depreciation for you. For 2025 the Â§168(k) bonus rate is 40% for property placed in service on or before Jan 19, 2025 and 100% for property placed in service after Jan 19, 2025 (OBBBA; IRS Notice 2026-11).'} wide />
          </label>
          ...
          <div>
            <label style={lbl}>
              ...
              <InfoTip text={isCCorp
                ? 'C-Corp owner-employees are paid a W-2 salary. The salary (and the employer-side payroll tax on it) is deductible to the corporation, reducing the profit subject to the 21% corporate tax. Reasonable-compensation rules still apply. The remaining after-tax corporate profit, when distributed, is taxed AGAIN as qualified dividends on your personal return â the classic C-Corp double taxation.'
                : 'S-Corp owners must pay themselves reasonable W-2 compensation for services rendered (Rev. Rul. 74-44). Too little salary is an audit trigger.\n\nA common starting point: 35â45% of your total S-Corp take (salary Ã· (salary + K-1 net income)). For example, if the S-Corp earns $200K net, a salary of $70Kâ$90K is a reasonable range â though the right number depends on industry, comparable wages, and time devoted.\n\nNote: "K-1 net income" here means ordinary business income (Box 1 of your K-1), not distributions. Distributions are cash drawn from the S-Corp and can differ from your share of net profit.\n\nPaying below-market salary:\nâ¢ IRS audit risk (Rev. Rul. 74-44)\nâ¢ Reduces your Â§199A W-2 wage limitation\nâ¢ Triggers the Reasonable Compensation Alert below\n\nFICA taxes (15.3% combined) apply to your W-2 salary â the K-1 business income that passes through is not subject to FICA or self-employment tax (whether or not it is distributed), which is the core S-Corp tax advantage.'} wide />
            </label>
            ...
            {officerExceedsRevenue && (
              <div style={{ fontSize: 12, color: R, marginTop: 4, fontWeight: 600 }}>
                â  Officer compensation exceeds gross receipts â verify your numbers.
              </div>
            )}
            {officerExceedsNetProfit && !officerExceedsRevenue && (
              <div style={{ fontSize: 12, color: '#D97706', marginTop: 4, fontWeight: 600 }}>
                â  Officer compensation exceeds net profit after operating expenses â this entity will show a net loss.
              </div>
            )}
        ...
        <div>
          <label style={lbl}>
            ...
            <InfoTip text="Miscellaneous business operating expenses not captured in the fields above. Must be ordinary and necessary under IRC Â§162. Exclude depreciation, advertising, and officer compensation â those have dedicated fields." />
          </label>
      ...
      {rv > 0 && (
        <div style={{ marginTop: 12, padding: '10px 14px', background: '#fff', borderRadius: 8, border: '1px solid #E2E8F0', fontSize: 13 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #E2E8F0', paddingTop: 6, fontWeight: 700 }}>
            <span style={{ color: N }}>{isRE ? FINANCIAL_LABELS.netRentalIncome : FINANCIAL_LABELS.netBusinessIncome}{isRE ? ' â Schedule E' : isPartnership ? ' â K-1 Box 1' : ''}</span>
      ...
      {isPartnership && (
        <div style={{ marginTop: 10, padding: '12px 14px', background: '#F0F9FF', borderRadius: 8, border: '1px solid #BAE6FD', fontSize: 12 }}>
          <div style={{ fontWeight: 700, color: '#0369A1', marginBottom: 6 }}>ð K-1 Box Mapping â Partnership / LLC</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5, color: '#334155' }}>
            <div>
              <span style={{ fontWeight: 600 }}>Box 1 â Ordinary Business Income:</span>{' '}
            ...
            <div style={{ borderTop: '1px solid #BAE6FD', paddingTop: 5 }}>
              <span style={{ fontWeight: 600 }}>Box 2 â Net Rental Income (Loss):</span>{' '}
              Rental income on this partnership K-1 (Box 2) is passive rental real estate on Schedule E, page 2 â it stays with this partnership entity rather than being entered as a separate Real Estate card. Passive activity rules (IRC Â§469) apply, including the $25K allowance and REP status.
            </div>
            <div style={{ borderTop: '1px solid #BAE6FD', paddingTop: 5 }}>
              <span style={{ fontWeight: 600 }}>Box 9a â Net Â§1231 Gain (Loss):</span>{' '}
              Enter K-1 Box 9a in the <span style={{ fontWeight: 700 }}>Capital Gains (Form 4797)</span> field in Step 2. Net Â§1231 gains are treated as long-term capital gain; net Â§1231 losses are ordinary.
            </div>
          </div>
        </div>
      )}
      ...
      {isRE && (
        <div style={{ marginTop: 10, padding: '12px 14px', background: '#F5F3FF', borderRadius: 8, border: '1px solid #DDD6FE', fontSize: 12 }}>
          <div style={{ fontWeight: 700, color: '#6D28D9', marginBottom: 4 }}>ð  Rental Property â Income, Expenses & Depreciation</div>
          <div style={{ fontSize: 11, color: '#6D28D9', marginBottom: 6, fontWeight: 500 }}>Schedule E Â· Passive activity rules (Â§469) apply</div>
          <div style={{ color: '#334155', lineHeight: 1.5 }}>
            Whether a net rental loss is deductible this year depends on your passive-activity status â Real Estate Professional (REP) status plus the Â§1.469-9(g) aggregation election makes the loss nonpassive, or the $25,000 active-participation allowance applies if you don't qualify as REP. Set your status on this card. Officer compensation doesn't apply to rentals.
          </div>
        </div>
      )}
...
// âââ Entity card ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function EntityCard({ entity, idx, onUpdate, onAggregationElection, portfolioAggregationElected, onRemove, colorAccent, isExpanded, onToggleExpand }) {
  ...
  // ââ Findings 1 + 3 FIX â single source of truth for S-Corp basis math ââââââââ
  // Mirrors taxCalc.js exactly so every badge and panel on this card agrees with the
  // engine that produces the filed return:
  //   â¢ Â§1367(a)(1): beginning stock basis (7203 Line 1) is raised by current-year
  //     capital contributions (Line 2) and basis-restoring income (Lines 3aâ3m), then
  //     by current-year income â BEFORE distributions or losses are applied.
  //   â¢ Â§1368 BEFORE Â§1366 (Reg. Â§1.1368-1(e)): distributions reduce that basis next;
  //     only the true excess over pre-loss basis is Â§1368(b)(2) long-term capital gain.
  //   â¢ Â§1366(d): the loss is then limited to whatever stock basis remains, plus debt
  ...
  const scBasis = (() => {
    ...
    // Â§1367(a)(1) income-first basis (a current-year loss is a Â§1366 item applied LAST).
    const stockBasisForDist = sb + contrib + basisInc + Math.max(0, k1Net)
    // Â§1368 excess gain only on the true excess over pre-loss basis (when basis is known).
    ...
    // Â§1366(d): loss capped by stock basis remaining AFTER distributions, plus debt basis.
  ...
  // ââ PASS4B-02b: Inline badge computations âââââââââââââââââââââââââââââââââ
  const basisBadge = (() => {
    if (!isSC || !scBasis) return null
    const { lossAmt, hasBasisInput, basisForLoss, suspendedLoss, dist } = scBasis
    if (lossAmt === 0) return null
    // C-10: a loss with no basis figure at all is conservatively suspended â prompt for basis.
    if (!hasBasisInput) {
      return { type: 'amber', msg: `Â§1366(d): enter stock basis â ${fmt(lossAmt)} loss may be limited.` }
    }
    if (suspendedLoss > 0) {
      const why = dist > 0
        ? `basis insufficient after Â§1368 distributions`
        : `basis insufficient`
      return { type: 'warn', msg: `Â§1366(d): ${fmt(suspendedLoss)} of your ${fmt(lossAmt)} loss is suspended â ${why}.` }
    }
    return { type: 'ok', msg: `Â§1366(d): Full ${fmt(lossAmt)} loss is deductible â within ${fmt(basisForLoss)} basis.` }
  })()
  ...
  const distBadge = (() => {
    ...
    if (!hasBasisInput) {
      return { type: 'amber', msg: `Â§1368: ${fmt(dist)} in distributions â enter stock basis to compute capital gain.` }
    }
    if (distExcessGain > 0) {
      return { type: 'warn', msg: `Â§1368: ${fmt(distExcessGain)} of distributions exceeds basis â treated as capital gain.` }
    }
    return { type: 'ok', msg: `Â§1368: All ${fmt(dist)} distributions are tax-free return of basis.` }
  })()
    ...
    <div style={{
      ...
      >
        <div style={{ width: 36, height: 36, borderRadius: 8, background: colorAccent, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>
          {/s.?corp/i.test(entity.type || '') ? 'ð¢' : /partner|mmllc/i.test(entity.type || '') ? 'ð¤' : /sole|single/i.test(entity.type || '') ? 'ð¼' : /c.?corp/i.test(entity.type || '') ? 'ðï¸' : 'ð'}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: SL }}>
            {entity.type} Â· {entity.own || 100}% ownership
            {entity.connectedId && <span style={{ marginLeft: 8, color: G, fontWeight: 600 }}>â Synced</span>}
            {entity.isManual && <span style={{ marginLeft: 8, color: '#D97706', fontWeight: 600 }}>â Manual</span>}
          </div>
        ...
        <div style={{ color: SL, fontSize: 14, flexShrink: 0 }}>{isExpanded ? 'â²' : 'â¼'}</div>
      </div>
      ...
      {isExpanded && (
        <div style={{ padding: '14px 18px', borderTop: '1px solid #F1F5F9' }}>
          ...
          {/* F-05 FIX: Ownership % â fully controlled input with 0â100 clamp */}
          <div style={{ marginBottom: 10 }}>
          ...
          {isPT && (
            <div style={{ marginBottom: 10 }}>
              <button onClick={e => { e.stopPropagation(); setShowQBI(s => !s) }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: B, padding: '4px 0', marginBottom: 6 }}>
                {showQBI ? 'â² Collapse' : 'â¼ Expand'} Â§199A Qualified Business Income (QBI) Deduction <span style={{ fontWeight: 500, opacity: 0.7 }}>Â· W-2 wages, UBIA, SSTB</span>
              </button>
              ...
              {!showQBI && (
                <div style={{ fontSize: 11, color: '#64748B', marginTop: -4, marginBottom: 6 }}>
                  {/* NEW-6 FIX: previous text said "only needed if taxable income exceeds
                      the threshold" â but the prior-year QBI loss carryforward must be
                      tracked even in LOSS years when current QBI deduction = $0.
                      Updated text covers both cases. IRC Â§199A(c)(2). */}
                  Only needed if your taxable income exceeds the Â§199A threshold â about $197,300 (single) or $394,600 (MFJ) for 2025 ($201,775 / $403,500 for 2026) â except Â§179 and charitable contributions, which always reduce QBI regardless of income level. <strong>Also complete this section in loss years</strong> â the QBI loss carryforward (Form 8995 Line 3) must be tracked for future years even when the current-year deduction is $0 (IRC Â§199A(c)(2)).
                </div>
              )}
              {showQBI && (
                <div style={{ background: '#EFF6FF', borderRadius: 8, padding: '12px 14px', border: '1px solid #BFDBFE' }}>
                  {/* NEW-1 FIX: SSTB advisory for businesses likely classified as SSTBs.
                      Teaching, tutoring, instruction, consulting etc. fall under Â§199A(d)(1)(B).
                      We warn when entity name/type contains SSTB keywords and the checkbox is
                      not yet checked â this is an advisory, not a determination. */}
                  {(() => {
                    ...
                    return (
                      <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8, padding: '10px 12px', marginBottom: 10, fontSize: 12, color: '#78350F' }}>
                        <strong>â  Possible Specified Service Trade or Business (SSTB) â review below.</strong>{' '}
                        Businesses involving teaching, tutoring, instruction, consulting, coaching, and similar services
                        commonly qualify as SSTBs under IRC Â§199A(d)(1)(B). If this is an SSTB, the QBI deduction
                  ...
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#1D4ED8', marginBottom: 10 }}>Â§199A QBI Inputs â from K-1</div>
                  {[
                    { label: 'W-2 Wages (K-1 Â§199A statement)',  /* UX-M4 FIX: shortened; box ref in tooltip */ key: 'box17V_wages', tip: 'Your share of W-2 wages paid by the entity. Reported on the Section 199A statement attached to your K-1 (S-Corp: Box 17, Code V; Partnership: Box 20, Code Z).\n\nThis field only matters if your taxable income exceeds ~$197,300 (single) or $394,600 (MFJ) for 2025 (~$201,775 / $403,500 for 2026). Below those thresholds, the W-2 wage limitation does not apply and your QBI deduction is simply 20% of QBI.\n\nAbove the threshold, the deduction is limited to the lesser of: (a) 20% of QBI, or (b) 50% of W-2 wages paid by the entity (IRC Â§199A(b)(2)(A)).' },
                    { label: 'UBIA of Qualified Property (K-1 Â§199A statement)',  /* UX-M4 FIX: shortened */ key: 'box17V_ubia', tip: 'Unadjusted Basis Immediately After Acquisition â the original cost of qualified property, not reduced by depreciation (IRC Â§199A(b)(6)(B)). Reported on the Section 199A statement attached to your K-1 (S-Corp: Box 17, Code V; Partnership: Box 20, Code Z).\n\nThis field only matters if your taxable income exceeds ~$197,300 (single) or $394,600 (MFJ) for 2025. Below those thresholds this limitation does not apply.\n\nAbove the threshold, you may use the alternative W-2 + UBIA limitation: 25% of W-2 wages plus 2.5% of UBIA (Â§199A(b)(2)(B)). This helps capital-intensive businesses with low W-2 wages.' },
                    { label: 'Â§179 Deduction (K-1 Box 11 / Box 12)', key: 'box11_12', tip: 'Â§179 first-year expensing allocated to you from the entity.\n\nS-Corp: K-1 Box 11 Â· Partnership: K-1 Box 12\n\nThis deduction reduces your Qualified Business Income (QBI) for Â§199A purposes (Treas. Reg. Â§1.199A-3(b)(1)(ii)(A)). It also reduces your stock or partnership basis (IRC Â§1367 / Â§705).\n\nOnly enter this if Â§179 is shown separately on your K-1 and is NOT already reflected in the ordinary business income on Box 1 (S-Corp) or Box 1 (Partnership). If your accounting software already netted Â§179 into your net profit figure, leave this blank to avoid double-counting.' },
                    { label: 'Charitable Contributions â K-1 Box 12 (S-Corp) or Box 13 (Partnership)', key: 'box12_13', tip: 'Charitable contributions passed through on your K-1 (S-Corp: Box 12; Partnership: Box 13). Enter totals from your own K-1 only â do not combine S-Corp Box 12 and Partnership Box 13 if you hold both entity types. These flow to Schedule A and also reduce your K-1 basis.' },
                    { label: 'Prior-Year QBI Loss Carryforward (Form 8995, Line 3)', key: 'qbiLossCarryforward', tip: 'If this entity generated a net QBI loss in the prior year, that loss must reduce this entity\'s QBI in the CURRENT year before computing the 20% deduction (IRC Â§199A(c)(2)).\n\nEnter the absolute value of last year\'s net QBI loss from this entity (as a positive number). From Form 8995 line 3 or Form 8995-A.\n\nTracking this per-entity (not pooled) is required by Treas. Reg. Â§1.199A-1(d)(2)(iii).' },
                  ].map(({ label, key, tip }) => (
          ...
          {/* PASS4B-02b: Â§1366(d) Stock Basis & Â§1368 Distributions */}
          {isSC && (
            <div style={{ marginBottom: 10 }}>
              <button
              ...
              >
                {showBasis ? 'â² Collapse' : 'â¼ Expand'} Stock & Debt Basis (Form 7203) <span style={{ fontWeight: 500, opacity: 0.7 }}>Â· Â§1366(d) loss limits & Â§1368 distributions</span>
              </button>
              {/* P3c FIX: contextual hint so users know when stock basis matters.
                  Stock basis limits loss deductibility â irrelevant if the entity
                  is profitable. Surfacing that context prevents users from
                  hunting for numbers they don't need. */}
              {!showBasis && (
                <div style={{ fontSize: 11, color: '#64748B', marginTop: -4, marginBottom: 6 }}>
                  Needed only if your S-Corp shows a loss â limits how much is deductible this year
                </div>
              )}
              {showBasis && (
                <div style={{ background: '#F5F3FF', borderRadius: 8, padding: '12px 14px', border: '1px solid #DDD6FE' }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#6D28D9', marginBottom: 10 }}>
                    Â§1366(d) Basis Limitation Â· Â§1368 Distributions â from Form 7203
                  </div>
                  ...
                  {/* Finding 3 FIX: basis-restoring income items (Form 7203, Lines 3aâ3m) â
                      income/tax-exempt items that raise stock basis under Â§1367(a)(1). */}
                  <div style={{ marginBottom: 10 }}>
                    <label style={{ fontSize: 11, fontWeight: 700, color: '#6D28D9', display: 'block', marginBottom: 3 }}>
                      Other Basis-Restoring Income (Form 7203, Lines 3aâ3m) â Optional
                      <InfoTip text={'Current-year income and tax-exempt items allocated to you that increase stock basis (Form 7203, Lines 3aâ3m) â for example separately stated income, interest, dividends, capital gains, or tax-exempt income.\n\nDo NOT include the ordinary business loss here (that is entered as the entity\'s P&L and applied last under Â§1366). Enter only positive basis-increasing items. These raise basis under Â§1367(a)(1) before distributions and losses.\n\nLeave blank if not applicable.'} wide />
                    </label>
                    <MoneyInput
                  ...
                  <div style={{ marginBottom: 10 }}>
                    <label style={{ fontSize: 11, fontWeight: 700, color: '#6D28D9', display: 'block', marginBottom: 3 }}>
                      Debt Basis (Form 7203, Part II) â Optional
                      <InfoTip text={'Debt basis arises from bona fide loans you have personally made to the S-Corp â NOT loans the corporation took from a bank or third party.\n\nDebt basis absorbs losses after stock basis is exhausted (IRC Â§1366(d)(1)(B)).\n\nThis is an advanced field. Most shareholders only have stock basis. Leave blank if you have not personally loaned money to your S-Corp.\n\nSee Form 7203, Part II for the calculation.'} wide />
                    </label>
                    <MoneyInput
                  ...
                  {(() => {
                    ...
                    // C-10: loss present but no basis figure at all â conservatively suspended.
                    if (!hasBasisInput) {
                      return (
                        <div style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A', borderRadius: 8, padding: '10px 12px', marginBottom: 10, fontSize: 12 }}>
                          <div role="alert" style={{ fontWeight: 700, color: '#78350F', marginBottom: 4 }}>â  Â§1366(d) â Enter Stock Basis</div>
                          <div style={{ color: '#78350F', lineHeight: 1.5 }}>
                            This entity shows a {fmt(lossAmt)} loss. Your deductible S-Corp loss is capped at your
                            combined stock + debt basis (IRC Â§1366(d)(1)). Because no beginning basis or contributions
                            have been entered, this {fmt(lossAmt)} loss is being conservatively suspended and carried
                            forward (Â§1366(d)(2)) rather than deducted against your other income. Enter your beginning
                    ...
                    if (suspendedLoss > 0) {
                      return (
                        <div style={{ background: '#FEF2F2', border: '1.5px solid #FECACA', borderRadius: 8, padding: '10px 12px', marginBottom: 10, fontSize: 12 }}>
                          <div role="alert" style={{ fontWeight: 700, color: R, marginBottom: 4 }}>â  Â§1366(d) Loss Limitation Active</div>
                          <div style={{ color: '#7F1D1D', lineHeight: 1.5 }}>
                            ...
                            {dist > 0 ? ' after this year\u2019s distributions reduce stock basis first (Â§1368 before Â§1366)' : ''}.
                            Only <strong>{fmt(allowedLoss)}</strong> is deductible this year.
                            <strong> {fmt(suspendedLoss)}</strong> is suspended and carries forward (IRC Â§1366(d)(2)).
                    ...
                    return (
                      <div style={{ background: '#F0FDF4', border: '1.5px solid #86EFAC', borderRadius: 8, padding: '10px 12px', marginBottom: 10, fontSize: 12 }}>
                        <div style={{ fontWeight: 700, color: '#166534', marginBottom: 4 }}>â Full Loss Deductible</div>
                        <div style={{ color: '#166534', lineHeight: 1.5 }}>
                          Your {fmt(lossAmt)} K-1 loss is within the {fmt(basisForLoss)} of basis available
                          {dist > 0 ? ' (after distributions are applied first under Â§1368)' : ''} â the full loss is deductible this year.
                        </div>
                      </div>
                    )
                  })()}
                  ...
                  <div style={{ marginBottom: 6 }}>
                    <label style={{ fontSize: 11, fontWeight: 700, color: '#6D28D9', display: 'block', marginBottom: 3 }}>
                      ...
                      <InfoTip text={'Total cash or property distributions you received from the S-Corp this year.\n\nDistributions reduce your stock basis. If distributions exceed your remaining stock basis (after the current year\'s income/loss allocation), the excess is taxable as a long-term capital gain (IRC Â§1368(b)(2)) â NOT ordinary income.\n\nThis capital gain does not appear on your K-1. It is computed at the shareholder level and belongs on Schedule D.\n\nAAA note: If your S-Corp has accumulated earnings and profits (E&P) from a prior C-Corp period, AAA ordering rules (Â§1368(c)) apply â distributions reduce AAA first, then E&P (taxable as an ordinary dividend). Consult your CPA if prior C-Corp E&P exists.'} wide />
                    </label>
                    <MoneyInput
                  ...
                  {(() => {
                    ...
                    if (!hasBasisInput) {
                      return (
                        <div style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A', borderRadius: 8, padding: '10px 12px', fontSize: 12 }}>
                          <div role="alert" style={{ fontWeight: 700, color: '#78350F', marginBottom: 4 }}>â  Enter Stock Basis to Compute Capital Gain</div>
                          <div style={{ color: '#78350F', lineHeight: 1.5 }}>
                            ...
                            as long-term capital gain (IRC Â§1368(b)(2)).
                          </div>
                        </div>
                      )
                    }
                    // Finding 1: Â§1368 is applied to pre-loss basis (income-first under
                    // Â§1367(a)(1)); a current-year loss does NOT reduce the basis that
                    // absorbs distributions, so the Â§1368(b)(2) gain is only the true excess.
                    if (distExcessGain > 0) {
                      return (
                        <div style={{ background: '#FEF2F2', border: '1.5px solid #FECACA', borderRadius: 8, padding: '10px 12px', fontSize: 12 }}>
                          <div role="alert" style={{ fontWeight: 700, color: R, marginBottom: 4 }}>â  Â§1368 Capital Gain â Distributions Exceed Basis</div>
                          <div style={{ color: '#7F1D1D', lineHeight: 1.5 }}>
                            ...
                            <strong>long-term capital gain</strong> on Schedule D (IRC Â§1368(b)(2)).
                            Distributions are applied to basis before the year's loss (Reg. Â§1.1368-1(e)),
                    ...
                    return (
                      <div style={{ background: '#F0FDF4', border: '1.5px solid #86EFAC', borderRadius: 8, padding: '10px 12px', fontSize: 12 }}>
                        <div style={{ fontWeight: 700, color: '#166534', marginBottom: 4 }}>â Distributions Within Basis</div>
                        <div style={{ color: '#166534', lineHeight: 1.5 }}>
                          All {fmt(dist)} in distributions are a tax-free return of basis â no capital gain triggered (IRC Â§1368(b)(1)).
          ...
          {/* REG-01 / F6: Â§469 Passive Activity Status â the single home for rental treatment */}
          {isRE && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ background: '#F5F3FF', borderRadius: 8, padding: '12px 14px', border: '1px solid #DDD6FE' }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#6D28D9', marginBottom: 10 }}>
                  Â§469 Passive Activity Status â Schedule E Rental
                </div>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 8 }}>
                  ...
                  <label htmlFor={'rep_' + idx} style={{ fontSize: 12, color: '#5B21B6', cursor: 'pointer', lineHeight: 1.4 }}>
                    Real Estate Professional (REP) â IRC Â§469(c)(7)
                    <InfoTip text={'Check this ONLY if you meet both IRC Â§469(c)(7) tests:\n(1) more than half of the personal services you perform in all trades or businesses during the year are in real property trades or businesses in which you materially participate, AND\n(2) you perform more than 750 hours of service in those real property trades or businesses.\n\nREP status alone does NOT make your rentals nonpassive â you must also make the Â§1.469-9(g) aggregation election below.'} wide />
                  </label>
                </div>

                {/* C-11: Â§469(c)(7)(B) quantitative hours test. Replaces blank self-attestation
                    ...
                    change passive treatment (the Â§1.469-9(g) election below is still the switch). */}
                {entity.isREP && (() => {
                  ...
                  const reasons  = []
                  if (!pass750) reasons.push(`you entered ${reHrs.toLocaleString()} real-property hours â the statute requires more than 750`)
                  ...
                  return (
                    <div style={{ background: '#fff', border: '1px solid #DDD6FE', borderRadius: 8, padding: '10px 12px', marginBottom: 8 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: '#6D28D9', marginBottom: 8 }}>
                        Â§469(c)(7)(B) qualification â enter your hours
                      </div>
                      ...
                      {!entered ? (
                        ...
                        <div style={{ fontSize: 12, color: '#166534', background: '#F0FDF4', border: '1px solid #86EFAC', borderRadius: 6, padding: '7px 10px', lineHeight: 1.5 }}>
                          â Meets the Â§469(c)(7)(B) test â {reHrs.toLocaleString()} real-property hours ({'>'} 750) and {pctRE}% of your {totalHrs.toLocaleString()} total hours ({'>'} 50%). Keep contemporaneous daily time logs to support this if examined.
                        </div>
                      ) : (
                ...
                {/* F6: Â§1.469-9(g) aggregation election â shown once REP is checked. This is the
                ...
                {entity.isREP && (
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 8 }}>
                    ...
                    <label htmlFor={'agg_' + idx} style={{ fontSize: 12, color: '#5B21B6', cursor: 'pointer', lineHeight: 1.4 }}>
                      Apply the aggregate-hours rule across all rental properties (Â§1.469-9(g) election)
                      <InfoTip text={'Elect to treat ALL your rental real estate as a single activity, counting your participation HOURS across every property together. Meeting material participation on the combined activity makes the whole rental portfolio nonpassive â losses offset other income with no Â§469(i) cap, and rental income is excluded from the 3.8% net investment income tax.\n\nThis is a deliberate, generally irrevocable election made on the return; it is never assumed. Leave it unchecked and your rentals stay passive (the default).\n\nNote: this is the Â§469 aggregation ELECTION, not the Â§199A 250-hour rental "safe harbor" (Rev. Proc. 2019-38), which only affects the QBI deduction.\n\nTreas. Reg. Â§1.469-9(g) Â· IRC Â§469(c)(7).'} wide />
                    </label>
                  </div>
                )}
                ...
                {!entity.isREP && (
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 4 }}>
                    ...
                    <label htmlFor={'active_' + idx} style={{ fontSize: 12, color: '#5B21B6', cursor: 'pointer', lineHeight: 1.4 }}>
                      Active Participant â Â§469(i) $25,000 allowance
                      <InfoTip text={'Active participation is a lower bar than material participation â it generally means you make bona fide management decisions (approving tenants, setting rental terms, approving expenses).\n\nIf you actively participate, up to $25,000 of rental losses may offset other (non-passive) income under IRC Â§469(i). This allowance phases out at 50 cents per dollar of modified AGI between $100,000 and $150,000.'} wide />
                    </label>
                  </div>
                )}

                {/* Prior-year passive loss carryforward (Form 8582) â moved here from Step 2 */}
                ...
                {(() => {
                  ...
                  if (reNet >= 0) {
                    return (
                      <div style={{ background: '#F0FDF4', border: '1.5px solid #86EFAC', borderRadius: 8, padding: '10px 12px', marginTop: 8, fontSize: 12 }}>
                        <div style={{ fontWeight: 700, color: '#166534', marginBottom: 4 }}>â Rental Income Flows Through</div>
                        <div style={{ color: '#166534', lineHeight: 1.5 }}>
                  ...
                  if (entity.isREP && entity.rentalAggregationElection === true) {
                    // ââ Finding 2 FIX ââââââââââââââââââââââââââââââââââââââââââââââ
                    // The Â§1.469-9(g) election alone no longer green-lights the loss when
                    // the taxpayer's own Â§469(c)(7)(B) hours FAIL the quantitative test.
                    ...
                    // Failed test, NOT overridden â suspended. No green state. Offer override.
                    if (hoursFail && !override) {
                      return (
                        <div style={{ background: '#FEF2F2', border: '1.5px solid #FECACA', borderRadius: 8, padding: '10px 12px', marginTop: 8, fontSize: 12 }}>
                          <div role="alert" style={{ fontWeight: 700, color: R, marginBottom: 4 }}>â  REP Hours Test Failed â Loss Suspended (Â§469(c)(7)(B))</div>
                          <div style={{ color: '#7F1D1D', lineHeight: 1.5, marginBottom: 8 }}>
                            Your entered hours don't meet the Â§469(c)(7)(B) test ({reHrs.toLocaleString()} real-property hours{totHrs > 0 ? `, ${pctRE}% of your ${totHrs.toLocaleString()} total` : ''} â the statute requires more than 750 hours <strong>and</strong> more than 50%). Because the test fails, the Â§1.469-9(g) election does not make this {fmt(Math.abs(reNet))} loss currently deductible â it is treated as passive and suspended on Form 8582. This is the single largest audit-exposure item on a return with significant non-real-estate W-2 income.
                          </div>
                        ...
                        <div style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A', borderRadius: 8, padding: '10px 12px', marginTop: 8, fontSize: 12 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                            ...
                            <label htmlFor={'repovr_' + idx} style={{ fontSize: 12, color: '#78350F', cursor: 'pointer', lineHeight: 1.4 }}>
                              Override active â deducting despite the failed hours test. Uncheck to suspend the loss.
                            </label>
                          </div>
                        </div>
                      )
                    ...
                    // Hours pass, or none entered (backward-compatible) â currently deductible.
                    return (
                      <div style={{ background: '#F0FDF4', border: '1.5px solid #86EFAC', borderRadius: 8, padding: '10px 12px', marginTop: 8, fontSize: 12 }}>
                        <div style={{ fontWeight: 700, color: '#166534', marginBottom: 4 }}>â Nonpassive â Currently Deductible (REP + Â§1.469-9(g) election)</div>
                        <div style={{ color: '#166534', lineHeight: 1.5 }}>
                          ...
                          {hoursPass
                            ? ` Your ${reHrs.toLocaleString()} real-property hours (${pctRE}% of total) meet the Â§469(c)(7)(B) test â keep contemporaneous daily time logs to support it.`
                            : ' Enter your Â§469(c)(7)(B) hours above to confirm this position â without them, the deduction is unverified and at risk on audit, especially alongside significant W-2 income.'}
                        </div>
                      </div>
                    )
                  }
                  if (entity.isREP) {
                    return (
                      <div style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A', borderRadius: 8, padding: '10px 12px', marginTop: 8, fontSize: 12 }}>
                        <div role="alert" style={{ fontWeight: 700, color: '#78350F', marginBottom: 4 }}>â  Passive Until You Elect (Â§1.469-9(g))</div>
                        <div style={{ color: '#78350F', lineHeight: 1.5 }}>
                          REP status alone does not make this {fmt(Math.abs(reNet))} loss deductible. Check the Â§1.469-9(g) aggregation election above to treat your portfolio as nonpassive; otherwise it is limited by the Â§469(i) $25,000 allowance and otherwise suspended on Form 8582.
                        </div>
                      </div>
                    )
                  }
                  return (
                    <div style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A', borderRadius: 8, padding: '10px 12px', marginTop: 8, fontSize: 12 }}>
                      <div role="alert" style={{ fontWeight: 700, color: '#78350F', marginBottom: 4 }}>â  Passive Loss â Suspended This Year (Â§469)</div>
                      <div style={{ color: '#78350F', lineHeight: 1.5 }}>
                        This {fmt(Math.abs(reNet))} rental loss is passive. Under IRC Â§469(a) it is suspended and carried forward on Form 8582{entity.isActiveParticipant ? ', except for any portion allowed by the Â§469(i) $25,000 active-participation allowance (which phases out between $100Kâ$150K MAGI)' : ''}.
                      </div>
                    </div>
                  )
                })()}
            ...
            >
              ...
              {entity.isManual
                ? (nf(entity.pnl?.grossRevenue) > 0 || nf(entity.pnl?.totalExpenses) > 0
                    ? 'â Edit Financials'
                    : 'â Enter Financials')
                : 'â³ Disconnect / reconnect software'}
            </button>
...
export default function CalculateTaxInner() {
  ...
  useEffect(() => {
    ...
    if (existingStep1.length > 0) {
      ...
      // the entity list via writeStep1State â which Step 2 reads through readStep1State â
  ...
  const addEntityOfType = useCallback((type) => {
    ...
    setEntities(prev => {
      // F-FUNC-07: a newly added rental inherits the portfolio-wide Â§1.469-9(g)
  ...
  // F-FUNC-07: the Â§1.469-9(g) aggregation election is a SINGLE taxpayer-level
  ...
  const persistStep1 = useCallback(() => {
    ...
    const k1Total = entities.reduce((s, e) => {
      ...
      // NOT as pass-through K-1 â so it must not be summed into k1Total. TaxReturn computes
  ...
  // FIX (k1-sync): keep the persisted Step-1 payload â entities AND the derived
  // k1Total â current on every edit, not only when "Save P&L"/"Continue" runs.
  // updateEntity previously refreshed only the sessionStorage working copy, so
  // editing a field that feeds k1Total (e.g. the QBI Â§179 box11_12) after an
  ...
  const handleSaveRecord = useCallback(async (name) => {
    ...
    // new id on every save â which previously forked an identically-named
    ...
    // Skipping the name (modal "Skip") passes null/'' â keep the record's prior
  ...
  // can display "Revenue updated: $X â $Y (+$Z)".
  async function fetchEntityPnL(idx, pid, tok, extra, isManualSync = false) {
    try {
      ...
      if (d && !d.error) {
          ...
          setEntities(prev => {
            ...
            if (isManualSync && updated[idx]) {
              ...
              if (prevRev !== newRev && prevRev > 0) {
                ...
                setSyncDiffs(s => ({
                  ...s,
                  [pid]: `Gross receipts updated: ${fmt(prevRev)} â ${fmt(newRev)} (${sign}${fmt(diff)})`
                }))
                // Clear diff after 8s
        ...
        setFooterError(`${pid.charAt(0).toUpperCase() + pid.slice(1)} sync failed${detail} â try Sync now or reconnect.`)
        setTimeout(() => setFooterError(null), 10000)
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8FAFC', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      {/* Mobile desktop nudge â shown on narrow viewports only via CSS media query equivalent.
          TaxStat360 is a desktop tool â the Tax Tracker has too many fields and a live
      ...
      <div className="ts360-mobile-nudge" style={{
        ...
        <span>ð» <strong>TaxStat360 works best on a desktop browser.</strong> Some features may be hard to use on a small screen.</span>
      </div>
      ...
      <nav style={{ background: '#fff', borderBottom: '1px solid #E2E8F0', padding: '0 16px', height: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100, minWidth: 0, overflowX: 'auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
            {[
              ...
              <div key={s.n} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                {i < 2 && <span style={{ color: '#CBD5E1', fontSize: 12 }}>âº</span>}
              </div>
            ))}
          </div>
        </div>
        <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
          ...
          <button onClick={() => navigate('/ai-analysis')} style={{ padding: '7px 12px', border: '1px solid #E2E8F0', borderRadius: 8, background: '#fff', fontSize: 12, cursor: 'pointer', color: isPro() ? SL : '#94A3B8', fontWeight: 600, whiteSpace: 'nowrap' }}>{STEP3_LABEL}{!isPro() ? ' ð' : ''}</button>
      ...
      <div style={{ maxWidth: 780, margin: '0 auto', padding: '32px 20px 120px' }}>
        ...
        {/* O1 FIX: The subtitle "or skip and enter manually" now has a working
            counterpart. An "Enter manually â" button appears in the integration
            ...
            copy is preserved â the path now fulfils the promise.
            CSV import has been removed entirely â entities are added by connecting
        ...
        <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 12, padding: '16px 18px', marginBottom: 18 }}>
          ...
          {/* F1 FIX (UX audit): the manual/express path was a small secondary button while
              the integration tiles dominated â but most first-time users (and real-estate
          ...
          <button
            onClick={() => {
              ...
              //      first â avoids hardcoding S-Corp for every user.
              ...
              if (blankIdx !== -1) {
                // Reuse the existing blank entity â just expand and scroll to it
              ...
              // No blank entity exists â gate on Pro before adding another
              if (!isPro() && entities.length >= 1) { navigate('/upgrade'); return }
          ...
          >
            <span style={{ fontWeight: 700, fontSize: 14 }}>â Enter my figures manually</span>
            <span style={{ fontWeight: 500, fontSize: 11, opacity: 0.85 }}>Fastest â type your gross receipts, expenses & salary</span>
          </button>
        ...
        {(isPro() || entities.length === 0) ? (
          ...
          <button onClick={() => navigate('/upgrade')} title="Multi-entity input is a Professional feature" style={{ width: '100%', padding: '13px', border: '2px dashed #BFDBFE', borderRadius: 12, background: '#F0F6FF', color: B, fontSize: 14, fontWeight: 700, cursor: 'pointer', marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            ð Add another entity â <span style={{ textDecoration: 'underline' }}>Professional</span>
          </button>
        )}
        <button onClick={() => addEntityOfType('Real Estate (Schedule E)')} style={{ width: '100%', padding: '13px', border: '2px dashed #A78BFA', borderRadius: 12, background: '#FAF5FF', color: '#6D28D9', fontWeight: 600, fontSize: 14, cursor: 'pointer', marginTop: 8 }}>{'ð  + Add Rental Property (Schedule E)'}</button>
        {/* UX-M3 FIX: unsaved warning also shown in sticky footer so it's always visible */}
        {entities.length > 0 && (<div style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A', borderRadius: 10, padding: '10px 14px', color: '#92400E', fontSize: 13, fontWeight: 500, marginTop: 12, display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 16 }}>{'â ï¸'}</span><span>{'Your entries are not saved yet. Click Save Progress below to keep them â unsaved work can be lost when you sign out or when accounting software re-syncs.'}</span></div>)}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, paddingRight: 80 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
            ...
            >
              ...
              {saveStatus === 'saving' ? 'Savingâ¦' : saveStatus === 'saved' ? 'â Saved' : 'Save Progress'}
            </button>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: '28px', maxWidth: 480, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                { type: 'S Corporation',            icon: 'ð¢', desc: 'K-1 income not subject to SE tax Â· reasonable officer W-2 compensation required'         },
                { type: 'Partnership / LLC',         icon: 'ð¤', desc: 'K-1 income Â· Schedule E page 2 Â· SE tax may apply'           },
                { type: 'Sole Proprietor / SMLLC',   icon: 'ð¼', desc: 'Schedule C Â· self-employment tax Â· QBI eligible'             },
                { type: 'Real Estate (Schedule E)',   icon: 'ð ', desc: 'Rental income/loss Â· passive activity rules Â· depreciation'  },
              ].map(({ type, icon, desc }) => (
                <button
                  key={type}
