...
// for runtime gating â Module 1 removed that import: entity routing now uses the shared
...
//         pattern â fixes silent 0%-ownership-treated-as-100% bug.
// UX-N02: Quarterly estimate in record card now includes safe harbor context.
//
// L-03 FIX: "S-CORP ALERT" badge changed to "â  AUDIT RISK â S-CORP" for clarity.
...
//           Previously amber (#FEF3C7 / #FCD34D / #92400E) â inconsistent severity.
//
...
// F24 FIX: Saved record cards displayed only the record name â no summary numbers
...
//     â¢ Est. federal tax liability (rec.totalTax)
//     â¢ Effective rate (rec.totalTax Ã· total income, estimated from saved fields)
//     â¢ Tax year (rec.taxYear || rec.biz.year)
...
//   non-zero totalTax. calcDashboard() already computed these values â they just
...
// Single-Member LLC", "Partnership / MMLLC â Active") while PASSTHROUGH_ENTITY_TYPES
// holds the UI-label form ("Sole Proprietor / SMLLC", "Partnership / LLC"), so the
// membership test returned false for sole proprietors and partnerships â passing an
...
export function calcDashboard(biz, f1040) {
  ...
  if (isCCorp) {
    ...
    // netBiz is profit BEFORE salary when derived from grossâopExp, but AFTER salary when it
    // comes from a synced pnl.netProfit â so reconstruct profit-before-salary either way.
...
function buildRecs(biz, calc) {
  ...
  if (isCCorp && corpTax > 0)
    recs.push({ type: 'danger', title: 'C-Corp Double Taxation', msg: `Your corporation owes ${fmt(corpTax)} in federal corporate tax (a flat 21% on profit after your officer compensation and employer payroll tax). The remaining ${fmt(dividends)} in after-tax profit, distributed as qualified dividends, is taxed again on your personal return â the classic double taxation. Consider an S-Corp election to eliminate the entity-level tax.` })
  ...
  if (qbi > 0)
    recs.push({ type: 'success', title: `QBI Deduction Applied â ${fmt(qbi)} Deduction`, msg: `You qualify for the 20% Â§199A deduction, reducing your taxable income by ${fmt(qbi)}.` })
  if (dep === 0 && grossRev > 50000)
    recs.push({ type: 'info', title: 'Review Depreciation Deductions', msg: 'No depreciation recorded. Equipment, vehicles, and home office may be deductible under Â§179.' })
  if (nf(effRate) > 28)
...
const ONBOARDING_STEPS = [
  ...
  { emoji: 'ð¢', badge: 'Step 1 of 2 â Business Entities', title: 'Add Your Business Entities', body: 'Connect QuickBooks, Xero, Wave, or FreshBooks â or enter revenue and expenses manually. K-1 income flows automatically to your personal return.' },
  { emoji: 'ð', badge: 'Step 2 of 2 â Personal Return', title: 'Complete Your Personal Return', body: 'Enter filing status, W-2 income, rental real estate, and deductions. Your federal tax liability, Â§199A QBI deduction, and quarterly estimated payments update live.' },
  { emoji: 'ð¤', title: 'AI Risk & Tax Analysis', body: 'Save your calculation to unlock your AI risk scan â officer compensation audit flags, penalty risk, QBI limits, and tax-saving strategies tailored to your situation.' },
  { emoji: 'â', title: "You're all set!", body: "Your Dashboard stores all your saved records. Load any record to update it, or start a new calculation anytime. Let's build your first one.", isFinal: true },
]
      ...
      <div style={{ background: '#fff', borderRadius: 20, padding: '36px 32px', maxWidth: 480, width: '100%', boxShadow: '0 24px 80px rgba(0,0,0,0.35)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {step > 0 && (
              <button onClick={() => setStep(step - 1)} style={{ padding: '10px 18px', borderRadius: 8, border: '1px solid #E2E8F0', background: '#fff', fontSize: 13, fontWeight: 600, color: SL, cursor: 'pointer' }}>â Back</button>
            )}
            {isLast ? (
              <button onClick={onComplete} style={{ padding: '10px 24px', borderRadius: 8, border: 'none', background: B, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Start Calculating â</button>
            ) : (
              <button onClick={() => setStep(step + 1)} style={{ padding: '10px 24px', borderRadius: 8, border: 'none', background: N, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Next â</button>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '10px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <span style={{ fontSize: 16 }}>ðºð¸</span>
        <span style={{ fontSize: 13, color: '#1e40af', fontWeight: 500 }}>
          <strong>Federal estimates only.</strong> TaxStat360 calculates federal income tax liability. State income tax is not included â add your state's effective rate separately for a complete picture.
        </span>
      </div>
      <button onClick={dismiss} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#64748B', fontSize: 18, lineHeight: 1, padding: 0 }} aria-label="Dismiss">Ã</button>
    </div>
  )
}
    ...
    if (this.state.hasError) {
      return (
        <div style={{ background: '#FEF2F2', border: '1px solid #FCA5A5', borderRadius: 8, padding: '12px 16px', fontSize: 12, color: '#991B1B' }}>
          â  This section failed to load. Reload the page to retry.
        </div>
      );
    }
...
export default function Dashboard() {
  ...
  // F-19 UX FIX: responsive nav â collapse labels to icons on narrow viewports
  ...
  useEffect(() => {
    ...
    if (xeroToken) {
      ...
      setXeroLoading(true)
      // Routes through apiClient â API_BASE_URL, consistent with every other auth call.
      // (Previously used a ts360_api_base localStorage override that was never set, so it
      // defaulted to same-origin â the wrong host under the split app/API origins.)
  ...
  const loadRecord = (rec) => {
    ...
    // the saved record and on the AI Schedule Map â so a loaded return showed (and was
    ...
    // coerced to numbers, with nothing merged in and nothing dropped â so the form,
  ...
  // hint it writes survives â clearStep1State clears any prior hint). The type
    ...
    <div style={{ fontFamily: 'Inter, system-ui, sans-serif', minHeight: '100vh', background: '#F8FAFC' }}>
      ...
      <nav style={{ background: '#fff', borderBottom: '1px solid #E2E8F0', padding: '0 28px', height: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100, overflowX: 'auto', minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={() => nav('/calculate-tax')} style={{ padding: '7px 16px', border: '1px solid #E2E8F0', borderRadius: 8, background: '#fff', fontSize: 13, cursor: 'pointer', color: SL, fontWeight: 600 }} title="Tax Tracker">{isMobile ? 'ð§®' : 'Tax Tracker'}</button>
          <button onClick={() => nav('/ai-analysis')}  style={{ padding: '7px 16px', border: '1px solid #E2E8F0', borderRadius: 8, background: isPro() ? '#fff' : '#f8fafc', fontSize: 13, cursor: isPro() ? 'pointer' : 'default', color: isPro() ? SL : '#cbd5e1', fontWeight: 600 }} title={isPro() ? 'AI Analysis & Reporting' : 'Upgrade to Professional to unlock AI Analysis & Reporting'}>{isMobile ? 'ð¤' : 'AI Analysis & Reporting'}{!isPro() && ' ð'}</button>
          ...
          <button onClick={() => nav('/settings')}     style={{ padding: '7px 16px', border: '1px solid #E2E8F0', borderRadius: 8, background: '#fff', fontSize: 13, cursor: 'pointer', color: SL, fontWeight: 600 }} title="Settings">{isMobile ? 'â' : 'Settings'}</button>
        </div>
      </nav>
      ...
      {showDisclaimer && (
        <div style={{ background: '#FFFBEB', borderBottom: '2px solid #F59E0B', padding: '12px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          <div style={{ fontSize: 13, color: '#92400E', lineHeight: 1.5 }}>
            <strong>â  Estimation Tool Only:</strong> TaxStat360 calculates tax estimates for planning purposes only. This is not professional tax advice. Consult a licensed CPA before filing.{' '}
            <a href="/terms" style={{ color: '#92400E', fontWeight: 700, textDecoration: 'underline' }}>View full disclaimer â</a>
          </div>
      ...
      {show2FANudge && (
        <div style={{ background: '#EFF6FF', borderBottom: '2px solid #93C5FD', padding: '12px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          <div style={{ fontSize: 13, color: '#1E40AF', lineHeight: 1.5 }}>
            <strong>ð Secure your account:</strong> Two-factor authentication (2FA) is not enabled. IRS Publication 4557 strongly recommends 2FA for tax software.{' '}
            <button onClick={() => nav('/settings')} style={{ background: 'none', border: 'none', padding: 0, color: '#1E40AF', fontWeight: 700, textDecoration: 'underline', cursor: 'pointer', fontSize: 13 }}>Enable 2FA in Settings â</button>
          </div>
      ...
      {xeroLoading && (
        <div style={{ background: '#EFF6FF', borderBottom: '1px solid #BFDBFE', padding: '12px 28px', fontSize: 13, fontWeight: 600, color: '#1D4ED8', textAlign: 'center' }}>
          Importing your Xero financialsâ¦ please wait
        </div>
      )}
        ...
        {!hasNumbers && !dismissedCompAlert && records.length > 0 && (
          <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 12, padding: '20px 24px', marginBottom: 24, display: 'flex', alignItems: 'flex-start', gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <button onClick={startNewCalc} style={{ padding: '8px 18px', background: B, color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>Open Tax Tracker â</button>
            </div>
          </div>
        )}
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: '#991B1B', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ background: '#FEE2E2', color: '#991B1B', borderRadius: 6, padding: '2px 8px', fontSize: 11, fontWeight: 800 }}>â  AUDIT RISK â S-CORP</span>
                Reasonable Compensation Below Practitioner Guideline
              </div>

              <div style={{ fontSize: 13, color: '#991B1B', marginBottom: 10, fontWeight: 600 }}>
                Formula: Salary Ã· (Salary + Distributions)
              </div>
              <div style={{
                ...
                <span style={{ color: '#991B1B', fontSize: 12 }}> (threshold: â¥40%)</span>
              </div>
              ...
              <div style={{ fontSize: 12, color: '#991B1B', lineHeight: 1.5, background: 'rgba(153,27,27,0.08)', borderRadius: 6, padding: '8px 12px' }}>
                <strong>Recommended action:</strong> Consider increasing your officer W-2 compensation to bring it within the 35â45% practitioner-recommended range. Discuss the appropriate amount with your CPA â the correct salary depends on your specific role, hours, industry, and comparable pay.{' '}
                <a href="https://www.irs.gov/businesses/small-businesses-self-employed/s-corporation-compensation-and-medical-insurance-issues" target="_blank" rel="noopener noreferrer" style={{ color: '#991B1B', textDecoration: 'underline', fontWeight: 600 }}>IRS guidance on S-Corp compensation â</a>
              </div>
            </div>
            <button
        ...
        {records.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', background: '#fff', borderRadius: 16, border: '1px solid #E2E8F0' }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 16 }}>
              {[
                { label: 'S-Corp Owner',          icon: 'ð¢', desc: 'Salary + K-1 income' },
                { label: 'Sole Proprietor',        icon: 'ð¼', desc: 'Schedule C self-employment' },
                { label: 'Real Estate Investor',   icon: 'ð ', desc: 'Rental income + depreciation' },
                { label: 'Partnership / LLC',      icon: 'ð¤', desc: 'K-1 distributive share' },
              ].map(p => (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {records.map((rec, i) => {
              ...
              const entityType     = rec.biz?.type || rec.biz?.entityType || rec.entityType || 'â'
              const taxYear        = rec.taxYear || rec.biz?.year || 'â'
              const filingStatus   = (rec.f1040?.filingStatus || rec.filingStatus || 'â').toUpperCase()
              ...
              // totalTax === 0 is ambiguous â the Dashboard was showing "Complete Step 2 for
              ...
              // totalTax Ã· approximate total income. We derive income from the saved
              ...
              return (
                <div key={rec.id || i} style={{
                  ...
                  {/* ââ F24 FIX: Summary strip â always visible, no loading required ââ */}
                    ...
                    {i === 0 && records[1] && (nf(records[1].totalTax)) > 0 && totalTax > 0 && (() => {
                      ...
                      return (
                        <span style={{ fontSize: 11, fontWeight: 700, color: delta > 0 ? '#DC2626' : '#16A34A' }}>
                          {delta > 0 ? 'â²' : 'â¼'} {fmt(Math.abs(Math.round(delta)))} vs prior record
                        </span>
                      )
                    })()}
                  </div>
