...
//
// ââ Architecture rule ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// This file  â permanent rates, ratios, structural values, and law-defined
//              thresholds that never change year-to-year (IRC rates, ERISA ages,
//              FICA structure, and statutory dollar amounts explicitly NOT
//              inflation-adjusted).
// taxCalc.js â year-specific dollar figures (brackets, thresholds, limits,
//              phase-outs) stored in the TAX_TABLES[year] object.
//
// Import from here â never hard-code these values in component or utility files.
// When a new tax year is released, only taxCalc.js TAX_TABLES needs updating.
// Add the new year to SUPPORTED_TAX_YEARS below â that one edit advances the
// dropdowns and the CURRENT_TAX_YEAR default simultaneously.
//
// ââ ENTITY-TYPE REPRESENTATION (two vocabularies by design) âââââââââââââââââââ
// 1. UI / input layer (Vocabulary A) â what the user picks and what gets stored:
//    'S Corporation' Â· 'Partnership / LLC' Â· 'Sole Proprietor / SMLLC' Â·
...
//
// 2. Engine-internal canonical form (Vocabulary B) â what normalizeEntityType()
//    emits and what the tax engine keys on:
//    'S Corporation' Â· 'Partnership / MMLLC â Active' Â· 'Partnership / MMLLC â
//    Passive' Â· 'Sole Proprietor / Single-Member LLC' Â· 'Real Estate (Schedule E)'
//    The Active/Passive split exists ONLY in this layer because SE treatment
//    depends on it (Â§1402(a)(13)) and cannot be expressed in the single UI label.
//
//    normalizeEntityType() (utils/entityPredicates.js) is the one-way bridge AâB.
...
//    isRealEstateEntity) â they match EITHER vocabulary.
//    RULE: never test an entity type with exact-string .includes() against an
//    array in the OTHER layer's vocabulary.
//
// ââ Historical audit notes âââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
//
// O6 FIX: Added PLAN_FEATURES map â one-line feature summary per plan tier.
...
// Keep these strings short (under 60 chars) â they render at 11px in a constrained card.
...
// Branded CloudFront URL â all components use this constant; do not hardcode the
...
// âââ CURRENT TAX YEAR ââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
//
// â  UPDATE ANNUALLY: when you add TAX_TABLES[2027] to taxCalc.js, add 2027 to
// SUPPORTED_TAX_YEARS below â that single edit advances both the dropdowns and the
...
// C-32:7.1 â single source of truth for the Step-3 ("AI Analysis & Reporting") label.
// Previously the step-3 breadcrumb read "AI Analysis" in Steps 1â2 but "AI Analysis &
...
// âââ CANONICAL FEATURE NAMES â single source of truth (audit Categories 8/9) ââ
...
// descriptions always match. Import these â never inline the feature name strings.
//
// Audit Risk: pricing page said "Audit Risk Indicators"; app tab said "Risk Scan"
// â canonical: FEATURE_AUDIT_RISK_SCAN (used in tab label AND pricing copy)
...
//   â canonical: FEATURE_WHATIF_SIMULATOR (used in tab label AND pricing copy)
...
// âââ FINANCIAL LINE LABELS â single source of truth (audit Categories B/C/D/F) â
...
// label change a one-line edit. Change a label HERE â never re-inline it in a component.
//
...
// editable operating subset) â they are different figures, not synonyms.
export const FINANCIAL_LABELS = {
  ...
  // TERMINOLOGY FIX for Schedule C / real estate â separate citation per form
  ...
  // TERMINOLOGY FIX 3.1: "Other Deductions" â "Other Operating Expenses" in entity P&L forms.
...
// âââ SUBSCRIPTION PLAN IDENTIFIERS ââââââââââââââââââââââââââââââââââââââââââ
// C-01 FIX: Canonical plan IDs stored in localStorage['ts360_plan'] by the auth Lambda.
// ALL plan-gate checks must use these constants â never inline string literals.
//
// Storage â display name mapping:
//   PLAN_IDS.STARTER      = 'basic'      â what the auth Lambda writes to localStorage
...
//
// â  Do NOT change the string VALUES â they must match what the Lambda writes.
...
// O6 FIX: One-line feature summary per plan â consumed by Onboarding.jsx SignupScreen
...
export const PLAN_FEATURES = {
  basic: '1 entity Â· core tax tracker Â· quarterly estimates',
  pro: '3 entities Â· AI analysis Â· CPA Export tools',
  enterprise: 'Unlimited entities Â· multi-user Â· priority support',
}

// âââ FICA â IRC Â§3101 / Â§3111 âââââââââââââââââââââââââââââââââââââââââââââââââ
...
export const FICA_SS_RATE = 0.062        // IRC Â§3101(a) / Â§3111(a) â 6.2% per side
export const FICA_MEDICARE_RATE = 0.0145 // IRC Â§3101(b) / Â§3111(b) â 1.45% per side

// âââ ADDITIONAL MEDICARE TAX â IRC Â§3101(b)(2) / Â§1401(b)(2) âââââââââââââââââ
// 0.9% surcharge on wages and SE income above the threshold.
// Employee-only â no employer match on this portion.
//
// Thresholds (statutory; NOT inflation-adjusted since ACA enactment â Â§3101(b)(2)):
//   Â§3101(b)(2)(A) â joint return â $250,000
//   Â§3101(b)(2)(B) â married filing separately â $125,000
//   Â§3101(b)(2)(C) â any other case (single, HOH) â $200,000
//
...
// NIIT, which has NO withholding mechanism â both taxes share the same dollar values
...
export const ADDITIONAL_MEDICARE_TAX_RATE = 0.009          // IRC Â§3101(b)(2) / Â§1401(b)(2)
export const ADDITIONAL_MEDICARE_TAX_THRESHOLD_MFJ = 250000  // IRC Â§3101(b)(2)(A)
export const ADDITIONAL_MEDICARE_TAX_THRESHOLD_MFS = 125000  // IRC Â§3101(b)(2)(B)
export const ADDITIONAL_MEDICARE_TAX_THRESHOLD_SINGLE = 200000 // IRC Â§3101(b)(2)(C)
export const ADDITIONAL_MEDICARE_TAX_THRESHOLD_HOH = 200000 // IRC Â§3101(b)(2)(C) â head of household (any other case)
...
// âââ NET INVESTMENT INCOME TAX (NIIT) â IRC Â§1411 ââââââââââââââââââââââââââââ
// 3.8% on the lesser of:
//   (a) net investment income, OR
//   (b) the amount by which MAGI exceeds the applicable threshold.
//
// Thresholds (statutory; NOT inflation-adjusted since ACA enactment â Â§1411(b)):
//   Â§1411(b)(1) â joint return or surviving spouse â $250,000
//   Â§1411(b)(2) â married filing separately â Â½ of Â§1411(b)(1) â $125,000
//     Note: the statute cross-references paragraph (1) rather than hardcoding $125,000.
//     The computed value is used here. If MFJ threshold ever changes, MFS = MFJ Ã· 2.
//   Â§1411(b)(3) â any other case (single, HOH, QSS) â $200,000
//
...
//
// No withholding mechanism â flows entirely through Form 8960 and estimated payments.
export const NIIT_RATE = 0.038              // IRC Â§1411(a)
export const NIIT_THRESHOLD_MFJ = 250000    // IRC Â§1411(b)(1) â joint return / surviving spouse
export const NIIT_THRESHOLD_MFS = 125000    // IRC Â§1411(b)(2) â married filing separately (Â½ of MFJ)
export const NIIT_THRESHOLD_SINGLE = 200000 // IRC Â§1411(b)(3) â single, HOH, and all other filers
export const NIIT_THRESHOLD_HOH = 200000    // IRC Â§1411(b)(3) â head of household (grouped with single)
export const NIIT_THRESHOLD_QSS = 250000    // IRC Â§1411(b)(1) â qualifying surviving spouse (grouped with joint return)

// âââ SELF-EMPLOYMENT TAX DEDUCTION â IRC Â§164(f) âââââââââââââââââââââââââââââ
// Above-the-line deduction equal to 50% of self-employment tax paid.
// Reduces AGI; applied on Schedule 1, Line 15 of Form 1040.
// Applies to sole proprietors and active partners only â S-Corp shareholder-employees
// pay FICA on W-2 wages (not SE tax) and do not use this deduction.
export const SE_TAX_DEDUCTION_RATE = 0.50  // IRC Â§164(f)

// âââ SE TAX NET EARNINGS FACTOR â IRC Â§1402(a)(12) âââââââââââââââââââââââââââ
...
// Formula: net SE income Ã 0.9235 = SE earnings subject to tax
// Then: SE earnings Ã SE tax rate (15.3% up to SS wage base, 2.9% above) = SE tax owed.
//
// FICA on W-2 wages does NOT use this factor â FICA applies to 100% of wages.
// When computing S-Corp FICA savings vs. sole-prop SE tax on the same distributions,
// the comparison must use 0.9235 Ã distributions Ã SE rate (not distributions Ã FICA rate)
// to avoid overstating the S-Corp advantage.
// See: taxCalc.js ficaSavings calculation (TC-10 fix).
export const SE_NET_EARNINGS_FACTOR = 0.9235  // IRC Â§1402(a)(12)

// âââ NET OPERATING LOSS â IRC Â§172(a)(2) (TCJA / OBBBA) âââââââââââââââââââââ
...
// planning default â if the user has a confirmed pre-2018 NOL they should note that
...
export const NOL_CARRYFORWARD_CAP_RATE = 0.80  // IRC Â§172(a)(2)

// âââ PASSIVE ACTIVITY LOSS â IRC Â§469(i) âââââââââââââââââââââââââââââââââââââ
// Â§469(i) active-participation special allowance: up to $25,000 in rental losses
...
//
// Phase-out mechanics (Â§469(i)(3)(A)):
//   Reduction = PAL_PHASE_OUT_RATE Ã max(0, AGI â PAL_PHASE_OUT_START)
//   Allowance = max(0, PAL_SPECIAL_ALLOWANCE_BASE â Reduction)
//   Allowance is fully eliminated when AGI â¥ PAL_PHASE_OUT_END.
//
// Thresholds: NOT inflation-adjusted â these are statutory dollar amounts in Â§469(i)(3)(A).
// MFS filers: $0 allowance regardless of AGI â Â§469(i)(4).
//
// Usage: REP (Real Estate Professional) status bypasses Â§469(i) entirely â
// REPs deduct unlimited rental losses against ordinary income if they materially
// participate (Â§469(c)(7)). PAL_* constants only apply to non-REP active participants.
export const PAL_SPECIAL_ALLOWANCE_BASE = 25000   // Â§469(i)(2) â max allowance
export const PAL_PHASE_OUT_START = 100000          // Â§469(i)(3)(A) â phase-out begins here
export const PAL_PHASE_OUT_END = 150000            // Â§469(i)(3)(A) â allowance = $0 at this AGI
export const PAL_PHASE_OUT_RATE = 0.50             // Â§469(i)(3)(A) â 50 cents per dollar of excess

// âââ S-CORP REASONABLE COMPENSATION â IRC Â§3121; Rev. Rul. 74-44 âââââââââââââ
...
//
// IMPORTANT â this is a scrutiny signal, NOT a safe harbor or statutory floor:
...
//     where officer took $24K salary on ~$200K total compensation (12% ratio â extreme case).
...
// circumstances determination â a salary could be reasonable at 35% or unreasonable at 55%.
...
// âââ S-CORP GROSS-RECEIPTS SALARY TEST â Rev. Rul. 74-44 / Watson (8th Cir. 2012) ââ
// Flags when officer salary falls below 30% of gross receipts â the revenue-to-salary
...
// compensation. Rev. Rul. 74-44; Treas. Reg. Â§1.162-7.
//
// â  This is a PLANNING HEURISTIC â not a statutory threshold. Advise clients to
...
export const SCORP_REVENUE_SALARY_THRESHOLD = 0.30  // Rev. Rul. 74-44 / Watson (8th Cir. 2012) â revenue ratio signal


// âââ S-CORP DEFAULT OFFICER SALARY FRACTION â Rev. Rul. 74-44 / BLS p25 âââââ
...
// The BLS p25 lookup was scoped for a future PR â this constant is the planning fallback.
//
// â  This is a PLANNING HEURISTIC, not a statutory floor. See also:
...
// âââ CORPORATE INCOME TAX â IRC Â§11 ââââââââââââââââââââââââââââââââââââââââââ
...
// âââ CHILD TAX CREDIT â IRC Â§24 âââââââââââââââââââââââââââââââââ
// Per-child credit amount lives in the year tables (taxCalc.js â ctc.perChild).
// Â§24(b)(2)/(h)(3): the credit is reduced by $50 for each $1,000 (or fraction) of
// modified AGI above these thresholds. Statutory under TCJA (P.L. 115-97); NOT
// inflation-adjusted.
export const CTC_PHASEOUT_THRESHOLD_MFJ = 400000   // Â§24(h)(3) â joint return / surviving spouse
export const CTC_PHASEOUT_THRESHOLD_OTHER = 200000 // Â§24(h)(3) â single, HOH, MFS, all other filers
export const CTC_PHASEOUT_STEP = 1000              // Â§24(b)(2) â excess measured per $1,000
export const CTC_PHASEOUT_REDUCTION_PER_STEP = 50  // Â§24(b)(2) â $50 reduction per $1,000 step

// âââ ALTERNATIVE MINIMUM TAX (AMT) â IRC Â§55(b)(1) âââââââââââââââââââââââââââ
...
// year-specific â see TAX_TABLES[year].amt.bracket26_28 in taxCalc.js.
// AMT exemptions and phase-out ranges are inflation-adjusted annually and live in
// TAX_TABLES[year].amt in taxCalc.js â they are NOT defined here.
// (2024 reference values: exemption $85,700 single / $133,300 MFJ;
// phase-out start $609,350 single / $1,218,700 MFJ.)
export const AMT_RATE_LOW = 0.26   // IRC Â§55(b)(1)(A) â 26% on AMTI up to bracket26_28
export const AMT_RATE_HIGH = 0.28  // IRC Â§55(b)(1)(B) â 28% on AMTI above bracket26_28

// âââ LONG-TERM CAPITAL GAINS & QUALIFIED DIVIDENDS â IRC Â§1(h) âââââââââââââââ
...
export const LTCG_RATE_LOW = 0.00   // IRC Â§1(h)(1)(B) â 0% tier
export const LTCG_RATE_MID = 0.15   // IRC Â§1(h)(1)(C) â 15% tier
export const LTCG_RATE_HIGH = 0.20  // IRC Â§1(h)(1)(D) â 20% tier

// Unrecaptured Section 1250 gain â IRC Â§1(h)(1)(E) (25% rate) / Â§1(h)(6) (definition)
...
export const UNRECAPTURED_1250_MAX_RATE = 0.25  // IRC Â§1(h)(1)(E), Â§1(h)(6)

// Collectibles gain â IRC Â§1(h)(4)
// Coins, art, antiques, gems, stamps â held more than 1 year.
// Taxed at max 28% (same ceiling applies: lesser of 28% or ordinary bracket rate).
export const COLLECTIBLES_MAX_RATE = 0.28  // IRC Â§1(h)(4)

// âââ Â§199A QUALIFIED BUSINESS INCOME (QBI) DEDUCTION âââââââââââââââââââââââââ
// IRC Â§199A; Treas. Reg. Â§1.199A-1 through Â§1.199A-6
//
// Step 1 â Tentative deduction per entity:
//   QBI_DEDUCTION_RATE Ã qualified business income (20% of QBI)
//
// Step 2 â W-2 wage / UBIA limitation (applies when taxable income > threshold):
//   Income threshold: TAX_TABLES[year].qbi.threshold
//   When fully phased in, per-entity combined QBI amount = LESSER of Step 1 OR:
//     GREATER of:
//       W2_WAGE_LIMIT_RATE Ã W-2 wages paid by the business [50% of W-2]
//       W2_WAGE_ALT_RATE  Ã W-2 wages + UBIA_RATE Ã UBIA [25% W-2 + 2.5% UBIA]
//   IRC Â§199A(b)(2); Treas. Reg. Â§1.199A-1(d)(2)
//
// Step 3 â Overall taxable income cap (final ceiling, applied after Step 2):
//   QBI_DEDUCTION_RATE Ã (taxable income â net capital gains)
//   IRC Â§199A(a)(2). This cap applies even when W-2 wages are high â last constraint.
//
// Step 4 â SSTB limitation: phases out at high income per TAX_TABLES[year].
//
// Â§199A(i) OBBBA minimum deduction (tax years beginning after 12/31/2025):
//   If active QBI â¥ $1,000, deduction = GREATER of regular calc or $400.
//   Dollar amounts are year-specific and in QBI_MIN_DEDUCTION / QBI_MIN_THRESHOLD (taxCalc.js).
//
// â  C-02 / F-02 note: empty-string pnl fields (netProfit = '') produce NaN via parseFloat.
...
export const QBI_DEDUCTION_RATE = 0.20   // IRC Â§199A(a) â 20% of QBI
export const W2_WAGE_LIMIT_RATE = 0.50   // IRC Â§199A(b)(2)(A) â 50% of W-2 wages
export const W2_WAGE_ALT_RATE = 0.25     // IRC Â§199A(b)(2)(B)(i) â 25% of W-2 wages
export const UBIA_RATE = 0.025           // IRC Â§199A(b)(2)(B)(ii) â 2.5% of UBIA
// UBIA = Unadjusted Basis Immediately After Acquisition
//        (the original cost basis of qualified property, not reduced
//        by depreciation â IRC Â§199A(b)(6)(B))

// âââ RETIREMENT PLANS âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// ââ SEP-IRA â IRC Â§408(k); Â§402(h) âââââââââââââââââââââââââââââââââââââââââââ
// Employer-only contribution. For S-Corp shareholder-employees:
//   - Contribution base = W-2 officer compensation ONLY
//   - K-1 distributions do NOT count as compensation â IRC Â§402(h)(2)(A)
//   - S-Corp makes the contribution at the entity level (deductible on Form 1120-S)
//   - Max contribution = lesser of (SEP_IRA_RATE Ã W-2) OR dollar limit in TAX_TABLES
//   - Deadline: entity tax filing date including extensions
//     â S-Corp (Form 1120-S): September 15 (NOT October 15 â see LBL-I01 audit fix)
//     â Sole Prop (Form 1040): October 15
export const SEP_IRA_RATE = 0.25  // 25% of W-2 compensation â IRC Â§402(h)(2)(A)
...
export const SEP_IRA_SOLE_PROP_EFFECTIVE_RATE = 0.20  // 0.25 / 1.25 â net-of-SE-tax effective rate

// ââ Solo 401(k) â IRC Â§401(k); Â§415(c); Â§404(a)(3) âââââââââââââââââââââââââââ
// Employer profit-sharing contribution rate (same as SEP-IRA).
// Employee elective deferral limit is year-specific â TAX_TABLES[year].retirement.solo401kDeferral.
// Combined total capped at TAX_TABLES[year].retirement.solo401kMax (Â§415(c) overall limit).
export const SOLO_401K_EMPLOYER_RATE = 0.25  // 25% of W-2 compensation â IRC Â§404(a)(3)

// ââ Retirement plan catch-up eligibility ages â SECURE 2.0 Act Â§109 ââââââââââ
// These are law-defined structural ages, not year-specific dollar limits.
// IRC Â§414(v)(2)(E) as amended by SECURE 2.0 (P.L. 117-328, enacted 2022-12-29).
// Standard catch-up: age â¥ 50 in the tax year.
// Super catch-up: ages 60â63 in the tax year; reverts to standard at age 64.
//   At age 64+: catch-up returns to standard $7,500 (2025) â the super catch-up
//   window is ONLY ages 60, 61, 62, 63 (inclusive). This is a common planning error.
// Dollar amounts are year-specific â TAX_TABLES[year].retirement.catchUp401k/catchUp401kSuper.
export const CATCHUP_AGE_STANDARD = 50      // IRC Â§414(v)(1) â standard catch-up start age
export const CATCHUP_AGE_SUPER_START = 60   // SECURE 2.0 Â§109 â enhanced catch-up window start
export const CATCHUP_AGE_SUPER_END = 63     // SECURE 2.0 Â§109 â enhanced catch-up window end (inclusive)
...
// UI / input vocabulary (Vocabulary A â "layer 1" in the representation note above).
...
// picks and what gets persisted. The engine does NOT key on these strings directly â it
...
// the one non-pass-through type here â it appears in ENTITY_TYPES (a selectable structure)
...
//
// â  When adding a new entity type: update this array, the Tax Tracker <select>, the
...
// classified consistently â run it after any change here.
...
// engine form â that mismatch is exactly the bug Module 1 fixed in Dashboard.jsx. For
...
// SE-subject entity types â ENGINE-INTERNAL form (Vocabulary B / layer 2), i.e. the
...
// normalized values. Do not "align" these to the ENTITY_TYPES labels â doing so would
// break the engine, which keys on the layer-2 strings.
//   â¢ Sole Proprietor / Single-Member LLC â always SE-subject (Schedule C).
//   â¢ Partnership / MMLLC â Active â SE-subject; â Passive â NOT (so the passive variant
//     is intentionally absent here). The Active/Passive split lives in layer 2 precisely
//     because the single UI 'Partnership / LLC' label cannot carry it; Â§1402(a)(13).
//   â¢ S-Corp (officer W-2 is FICA-taxed instead) and Real Estate (passive rental) are
...
export const SE_SUBJECT_TYPES = [
  ...
  'Partnership / MMLLC â Active',
]

// âââ ACCOUNTING SOFTWARE INTEGRATIONS âââââââââââââââââââââââââââââââââââââââââ
// abbr values are displayed as badge text on integration logo tiles (Landing.jsx, Onboarding.jsx).
// LBL-01 fix: Xero corrected from 'XE' â 'X' (XE is the currency converter XE.com, not Xero).
// Wave corrected from 'WV' â 'W' (WV is non-standard; Wave's own mark uses 'W').
...
// (audit F-09, June 2026). Import integrationKey() from there â never from this file.
// Credential helpers are environment-specific; tax constants are not.

// âââ SUBSCRIPTION PRICING âââââââââââââââââââââââââââââââââââââââââââââââââââââ
// Monthly base prices â displayed on Landing.jsx pricing section and Upgrade.jsx.
// Annual pricing = monthly Ã ANNUAL_BILLING_MONTHS (10 months billed, 2 months free).
...
export const ANNUAL_DISCOUNT_LABEL = 'Save 2 months'  // display copy â update if discount changes

// âââ IRS STANDARD MILEAGE RATES âââââââââââââââââââââââââââââââââââââââââââââââ
...
//
// â  ANNUAL UPDATE REQUIRED: verify 2026 rate against IRS.gov/newsroom each December.
...
export const IRS_MILEAGE_RATES = {
  2024: 0.67,   // IRS Notice 2024-08 â 67Â¢/mile for business use
  2025: 0.70,   // IRS Notice 2025-05 â 70Â¢/mile for business use (5Â¢ increase from 2024)
  2026: 0.725,  // IRS Notice 2026-10 (Dec 29, 2025) â 72.5Â¢/mile for business use (up 2.5Â¢ from 2025)
}

// âââ COMPANY IDENTITY / NAP â footer + local SEO âââââââââââââââââââââââââââââ
...
// site-wide is a local-SEO signal â do not hardcode the address in individual pages.
...
// âââ CANONICAL DISCLAIMER â single source of truth âââââââââââââââââââââââââââ
// Audit fix (Pass 5, "Disclaimer wording varies"): the site carried at least three
// disclaimer strings â the Landing/About footer (full), the Privacy/Terms footer
...
// DISCLAIMER_FULL  â footers and standalone disclaimer boxes
// DISCLAIMER_SHORT â tight inline spots (e.g. the pricing-section banner)
// NOTE: this is consumer-facing legal copy. Edit it HERE only; it is owner-approved text.
export const DISCLAIMER_FULL = 'TaxStat360 is a tax planning and estimation tool â not a tax preparation or filing service. Calculations cover federal tax only (state taxes are not included) and are for planning purposes only. This is not professional tax, legal, or financial advice. Consult a licensed tax professional before making any filing or financial decisions.'
export const DISCLAIMER_SHORT = 'Planning and estimation tool â not tax preparation or filing. Federal tax only. Not professional tax advice.'

// âââ MARKETING CTA COPY âââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// #4 FIX: single source of truth for the trial CTA label + microcopy. Previously
// hand-written per page (Landing, About, Nav, Terms, ResourcesHub) and it drifted â
...
//
// â  Do NOT soften CTA_COPY_* to "card for verification only." The card IS the billing
...
// CTA_COPY_FULL  â used in the Landing hero
// CTA_COPY_SHORT â used in pricing, bottom CTA, About, and ResourcesHub
export const CTA_LABEL = 'Start Free 7-Day Trial'
export const CTA_COPY_FULL = 'No charge during your 7-day trial Â· Card required Â· Cancel in one click'
export const CTA_COPY_SHORT = 'No charge for 7 days Â· Card required Â· Cancel in one click'
