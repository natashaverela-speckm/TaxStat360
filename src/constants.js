...
// Thresholds: $200,000 single / $250,000 MFJ / $125,000 MFS (not inflation-adjusted).
// Applied in calcTaxReturn; thresholds stored in TAX_TABLES[year].additionalMedicareThreshold.
...
//
// Deduction = 20% of qualified business income (QBI), subject to:
//   1. Income threshold test (TAX_TABLES[year].qbi.threshold):
//      - Below threshold → full 20% deduction, no W-2 wage test required.
//      - Above threshold → W-2 wage / UBIA limitation phases in fully.
//   2. W-2 wage limitation (applies when taxable income > threshold):
//      Deduction is the LESSER of:
//        (a) QBI_DEDUCTION_RATE × QBI                           [20% of QBI]
//        (b) the GREATER of:
//              W2_WAGE_LIMIT_RATE × W-2 wages                   [50% of W-2]
//              W2_WAGE_ALT_RATE × W-2 wages + UBIA_RATE × UBIA [25% W-2 + 2.5% UBIA]
//   3. SSTB limitation: specified service trades or businesses lose the deduction
//      entirely above the threshold (phase-out range in TAX_TABLES[year].qbi).
//
...
//   solo401kMax      — §415(c) overall Solo 401(k) limit ($70,000 for 2025)
//   catchUp401k      — catch-up contribution age ≥ 50 ($7,500 for 2025)
...
export const SEP_IRA_RATE = 0.25   // 25% of W-2 compensation — IRC §408(k)(3)(A)
...
//     Limit: TAX_TABLES[year].retirement.solo401kDeferral ($23,500 for 2025)
//     + catch-up if age ≥ 50: TAX_TABLES[year].retirement.catchUp401k ($7,500 for 2025)
