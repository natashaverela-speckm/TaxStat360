...
describe('calcCCorpCorporateLayer — shared corporate-only figures', () => {
  it('matches the corporate-tax model (21% of profit after salary + employer FICA)', () => {
    const netProfit = 200000, officerSalary = 40000
    const empFICA = expectedEmployerFICA(officerSalary)
    const base = Math.max(0, netProfit - officerSalary - empFICA)
    const layer = calcCCorpCorporateLayer({ netProfit, officerSalary, taxYear: YEAR })
    expect(layer.employerFICA).toBe(empFICA)
    expect(layer.profitBeforeTax).toBe(base)
    expect(layer.corpTax).toBe(Math.round(base * C_CORP_TAX_RATE))
    expect(layer.dividends).toBe(layer.profitBeforeTax - layer.corpTax)
  })

  it('defaults and caps the officer salary the same way', () => {
    expect(calcCCorpCorporateLayer({ netProfit: 150000, taxYear: YEAR }).officerSalary)
      .toBe(Math.round(150000 * DEFAULT_OFFICER_SALARY_FRACTION))
    expect(calcCCorpCorporateLayer({ netProfit: 50000, officerSalary: 999999, taxYear: YEAR }).officerSalary)
      .toBe(50000)
  })

  it('is the single source of truth: calcCCorpReturn reuses identical corporate figures', () => {
    const netProfit = 175000, officerSalary = 55000
    const layer = calcCCorpCorporateLayer({ netProfit, officerSalary, taxYear: YEAR })
    const full  = calcCCorpReturn({ netProfit, officerSalary, personalContext: ctx })
    expect({
      officerSalary: full.officerSalary, employerFICA: full.employerFICA,
      profitBeforeTax: full.profitBeforeTax, corpTax: full.corpTax, dividends: full.dividends,
    }).toEqual(layer)
  })
})
