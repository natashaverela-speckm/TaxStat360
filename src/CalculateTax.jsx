...
function calcTax(biz, f1040) {
  ...
  // SE tax for passthrough (not S-Corp wages, but on K-1)
  ...
  const deduction = useStd ? stdDed : Math.max(stdDed, itemized)
  ...
  for (const [ceil,rate] of (BRACKETS[fs]||BRACKETS.single)) {
    ...
    incomeTax += (Math.min(taxableInc,ceil)-prev)*rate
    prev = ceil
  }
  ...
  const totalBeforeCredits = incomeTax + seTax
  const totalAfterCredits = Math.max(0, totalBeforeCredits - ctc)
  const taxOwed = Math.max(0, totalAfterCredits - estPay)
  const refund = Math.max(0, estPay - totalAfterCredits)
...
function buildRecs(biz, t) {
  ...
  if (isSCorp(biz.entityType) && officerSal === 0 && t.k1 > 20000)
    recs.push({type:'danger', title:'No Officer Compensation Recorded', msg:'S-Corp owners must pay themselves a reasonable salary before taking distributions. The IRS treats this as a top audit trigger. Based on your net income, we recommend at least '+fmt(t.recOfficerSal)+'/yr.'})
  if (isSCorp(biz.entityType) && officerSal > 0 && officerSal < t.recOfficerSal && t.k1 > 20000)
    recs.push({type:'warning', title:'Officer Compensation May Be Too Low', msg:'Your officer salary of '+fmt(officerSal)+' is below the IRS-recommended minimum of '+fmt(t.recOfficerSal)+' for your net income level. Consider increasing to reduce audit risk.'})
  if (t.quarterly > 500)
    recs.push({type:'warning', title:'Quarterly Estimated Payments Required', msg:'To avoid underpayment penalties, make quarterly payments of approximately '+fmt(t.quarterly)+'. Due: Apr 15, Jun 15, Sep 15, Jan 15.'})
  if (t.qbi > 0)
    recs.push({type:'success', title:'QBI Deduction Applied — You Saved '+fmt(t.qbi), msg:'As a passthrough entity owner, you qualify for the 20% Section 199A QBI deduction. This reduced your taxable income by '+fmt(t.qbi)+'.'})
  if ((parseFloat(biz.depreciation)||0)===0 && (parseFloat(biz.grossRevenue)||0)>50000)
    recs.push({type:'info', title:'No Depreciation Recorded', msg:'If you use equipment, a vehicle, or a home office for business, you may be missing significant deductions. Ask your CPA about Section 179 or bonus depreciation.'})
  if (t.k1 > 160000 && !isSCorp(biz.entityType) && isPassthrough(biz.entityType))
    recs.push({type:'info', title:'S-Corp Election Could Save You Money', msg:'At your income level, electing S-Corp status could save you thousands in self-employment taxes by splitting your income between salary and distributions.'})
  if (parseFloat(t.effectiveRate) > 28)
    recs.push({type:'warning', title:'High Effective Tax Rate ('+t.effectiveRate+'%)', msg:'Consider maximizing retirement contributions: SEP-IRA (up to $66,000), Solo 401k (up to $69,000), or HSA contributions to reduce taxable income.'})
  if (recs.length === 0)
    recs.push({type:'success', title:'Your Tax Structure Looks Healthy', msg:'No significant issues detected. Keep monitoring quarterly and update your numbers as your financials change.'})
  return recs
}
...
function InfoBox({type, title, msg}) {
  const s = {
    danger:  {bg:'#FEF2F2',border:'#FCA5A5',tc:'#991B1B',ic:'🚨'},
    warning: {bg:'#FFFBEB',border:'#FDE68A',tc:'#92400E',ic:'⚠️'},
    success: {bg:'#F0FDF4',border:'#86EFAC',tc:'#166534',ic:'✅'},
    info:    {bg:'#EFF6FF',border:'#BFDBFE',tc:'#1E40AF',ic:'ℹ️'},
  }[type]||{bg:'#F8FAFC',border:'#E2E8F0',tc:SL,ic:'ℹ️'}
  return (
...
export default function CalculateTax() {
  ...
  const [biz, setBiz] = useState({
    year: 2025, entityType:'S-Corporation',
    grossRevenue:'', businessExpenses:'', officerSalary:'', depreciation:'', otherDeductions:'', ownershipPct:'100'
  })
  ...
  useEffect(()=>{
    ...
    // If records exist, jump straight to analysis
      ...
      <div style={{maxWidth:640,margin:'40px auto',padding:'0 20px'}}>
        ...
        <div style={{background:'#fff',borderRadius:14,padding:24,boxShadow:'0 1px 8px rgba(0,0,0,0.06)',marginBottom:16}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            {INTEGRATIONS.map(i=>(
              <button key={i.id} onClick={()=>{window.open(API+'/integrations/'+i.id+'/connect','_blank'); localStorage.setItem('ts360_connected_app',i.name); setConnectedApp(i.name); setStep(2)}}
                style={{display:'flex',alignItems:'center',gap:12,padding:'13px 16px',background:i.bg,border:'1.5px solid '+i.color+'33',borderRadius:12,cursor:'pointer',textAlign:'left'}}
                onMouseOver={e=>e.currentTarget.style.borderColor=i.color}
                onMouseOut={e=>e.currentTarget.style.borderColor=i.color+'33'}
              >
                <div style={{width:38,height:38,borderRadius:9,background:i.color,color:'#fff',fontWeight:800,fontSize:13,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>{i.abbr}</div>
      ...
      {connectedApp && (
        <div style={{maxWidth:640,margin:'12px auto 0',padding:'0 20px'}}>
          <div style={{background:'#F0FDF4',border:'1px solid #86EFAC',borderRadius:10,padding:'9px 14px',fontSize:13,color:'#166534'}}>
            <strong>Connected: {connectedApp}</strong> — Review your imported numbers below.
          </div>
        </div>
      )}
      <Card>
        ...
        {isSCorp(biz.entityType) && (
          <Field label="Your Officer Salary" hint="Required for S-Corps — the IRS mandates you pay yourself a reasonable wage before taking distributions. Not optional.">
        ...
        <Card>
          ...
          <div style={{background:'linear-gradient(135deg,#1E3A5F,#1D4ED8)',borderRadius:12,padding:20,marginBottom:24,color:'#fff'}}>
            ...
            <div style={{fontSize:12,color:'#BFDBFE',lineHeight:1.6}}>
              This is your {biz.ownershipPct}% share of your business profit. It flows to your personal tax return (Schedule E, Line 17) — but it is <strong>not</strong> your tax bill. Your actual taxes depend on your total personal income, filing status, deductions, and credits below.
            </div>
          </div>
          ...
          <p style={{color:SL,fontSize:12,margin:'0 0 20px'}}>This mirrors your Form 1040. Add any income or credits from your personal life.</p>
          ...
          <Field label="W-2 Wages / Salary Income" hint="Any salary you or your spouse earn from employment (separate from your business)">
            <NumberInput value={f1040.w2Income} onChange={v=>fSet('w2Income',v)}/>
          </Field>

          <Field label="Other Personal Income" hint="Interest, dividends, rental income, capital gains, alimony received, or any other taxable income">
            <NumberInput value={f1040.otherIncome} onChange={v=>fSet('otherIncome',v)}/>
          </Field>

          <Field label="Number of Qualifying Children" hint="Children under 17 who qualify for the Child Tax Credit ($2,000 per child, subject to income limits)">
          ...
          <Field label="Deduction Method">
            <div style={{display:'flex',gap:8,marginBottom: f1040.useStandardDed?0:12}}>
              <button onClick={()=>fSet('useStandardDed',true)} style={{flex:1,padding:'10px',background:f1040.useStandardDed?B:'#fff',color:f1040.useStandardDed?'#fff':SL,border:'1.5px solid '+(f1040.useStandardDed?B:'#E2E8F0'),borderRadius:9,fontWeight:600,fontSize:13,cursor:'pointer'}}>
                Standard Deduction ({fmt(STD_DED[f1040.filingStatus])})
              </button>
            ...
            {!f1040.useStandardDed && (
              <NumberInput value={f1040.itemizedDed} onChange={v=>fSet('itemizedDed',v)} placeholder="Total itemized deductions"/>
            )}
          </Field>

          <Field label="Estimated Tax Payments Made This Year" hint="Quarterly payments you've already sent to the IRS — these reduce what you owe">
      ...
      <div style={{maxWidth:1060,margin:'0 auto',padding:'24px 20px'}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:20,flexWrap:'wrap',gap:10}}>
          ...
          <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
            ...
            <button onClick={()=>handleDelete(r.id)} style={{padding:'7px 14px',background:'#FEF2F2',color:'#DC2626',border:'none',borderRadius:8,fontWeight:600,fontSize:13,cursor:'pointer'}}>
              Delete
            </button>
          </div>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:18}}>
          ...
          <div>
            ...
            <div style={{background:'#fff',borderRadius:14,padding:22,boxShadow:'0 1px 6px rgba(0,0,0,0.05)',marginBottom:16}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                {[
                  ['Revenue',r.biz.grossRevenue],
                  ['Expenses',r.biz.businessExpenses],
                  ['Officer Salary',r.biz.officerSalary],
                  ['Depreciation',r.biz.depreciation],
                ].map(([l,v])=>(
              ...
              <div style={{background:'linear-gradient(135deg,#1E3A5F,#1D4ED8)',borderRadius:12,padding:18,marginTop:14,color:'#fff'}}>
                ...
                <div style={{fontSize:12,color:'#BFDBFE',lineHeight:1.6}}>
                  Your {r.biz.ownershipPct}% ownership share flows to Schedule E on your Form 1040. This is <strong>not</strong> your tax bill — it is one input into your personal return below.
                </div>
              </div>
            </div>
          ...
          <div>
            <div style={{background:'#fff',borderRadius:14,padding:22,boxShadow:'0 1px 6px rgba(0,0,0,0.05)',marginBottom:16}}>
              ...
              {[
                ...
                <div key={l} style={{display:'flex',justifyContent:'space-between',padding:'7px 12px',fontSize:13,borderBottom:'1px solid #F1F5F9'}}>
                  <span style={{color:SL}}>{l}</span>
                  <span style={{fontWeight:700,color:v<0?'#059669':c}}>{v<0?'-'+fmt(-v):fmt(v)}</span>
                </div>
              ))}
            ...
            {tax.quarterly>0 && (
              <div style={{background:'#fff',borderRadius:14,padding:22,boxShadow:'0 1px 6px rgba(0,0,0,0.05)'}}>
                ...
                <p style={{fontSize:11,color:'#94A3B8',margin:'12px 0 0',lineHeight:1.5}}>Pay at IRS.gov/payments — Direct Pay or EFTPS. Underpayment penalty applies if less than 90% of current year tax is paid.</p>
              </div>
            )}
          </div>
