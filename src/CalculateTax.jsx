...
// ─── Tax calculation engine ───────────────────────────────────────────────────
...
const styles = {
  ...
  intCard:   { display:'flex', alignItems:'center', gap:14, padding:'16px 20px', borderRadius:10, border:'2px solid transparent', cursor:'pointer', transition:'all 0.15s', background:'#F8FAFC' },
  ...
  intCheck:  { marginLeft:'auto', fontSize:18 },
  ...
  bigNum:      { fontSize:36, fontWeight:800, color:N },
...
export default function CalculateTax() {
  ...
  // Connection state — read from localStorage (set by OAuthCallback in App.jsx)
  ...
  // On mount — read any connected providers from localStorage
  useEffect(() => {
    const conn = {}
  ...
  // ── OAuth redirect — this is the real fix ──────────────────────────────────
  function handleConnect(provider) {
    // Save return state so we know what was connecting
    localStorage.setItem('ts360_connecting', provider)
    // Redirect to API Gateway OAuth start endpoint
    window.location.href = `${LAMBDA}/${provider}`
  }

  // ── Simulate sync (placeholder until real data API is built) ──────────────
  async function handleSync(provider) {
    if (!connected[provider]) { handleConnect(provider); return }
    setSyncing(provider)
    await new Promise(r => setTimeout(r, 1800))
    // Demo: pre-fill with sample data tagged to provider
    const demos = {
      quickbooks: { grossRevenue:'285000', businessExpenses:'94000', officerSalary:'72000', depreciation:'8500', otherDeductions:'4200', ownershipPct:'100' },
      xero:       { grossRevenue:'195000', businessExpenses:'62000', officerSalary:'55000', depreciation:'5000', otherDeductions:'2800', ownershipPct:'100' },
      wave:       { grossRevenue:'112000', businessExpenses:'38000', officerSalary:'40000', depreciation:'2200', otherDeductions:'1400', ownershipPct:'100' },
      freshbooks: { grossRevenue:'148000', businessExpenses:'49000', officerSalary:'48000', depreciation:'3100', otherDeductions:'1900', ownershipPct:'100' },
    }
    setBiz(prev => ({ ...prev, ...(demos[provider]||{}) }))
    setSyncing(null)
    setShowManual(true)
  }
  ...
  // ── Integration section ───────────────────────────────────────────────────
  const IntegrationSection = () => (
    <div style={styles.syncBox}>
          ...
          return (
            ...
            >
              <div style={{ ...styles.intAbbr, background: int.color }}>
                {isSyncing ? '⟳' : int.abbr}
              </div>
              <div>
                <div style={styles.intName}>{int.name}</div>
                {isConn && (
                  <span style={styles.connBadge}>
                    ✓ Connected{isSyncing ? ' — Syncing…' : ' — Click to sync'}
                  </span>
                )}
                {!isConn && (
                  <span style={{ fontSize:12, color:SL }}>
                    {isSyncing ? 'Connecting…' : 'Click to connect'}
                  </span>
                )}
              </div>
              {isConn && <span style={styles.intCheck}>✓</span>}
            </button>
          )
        })}
      </div>
      ...
      <p style={{ textAlign:'center', fontSize:12, color:'#94A3B8', marginTop:16, marginBottom:0 }}>
        Secure OAuth connection — your credentials are never stored
      </p>
    </div>
  )

  // ── Manual form ───────────────────────────────────────────────────────────
  const ManualForm = () => (
    <>
      {/* Business Section */}
      ...
      <div style={styles.formBox}>
        ...
        <div style={styles.fieldGrid}>
          <div>
            <label style={styles.label}>Other Income (interest, dividends…)</label>
          ...
          <div>
            ...
            <input style={styles.input} type="number" placeholder="0" min="0" max="10" value={f1040.dependents} onChange={e=>setF1040({...f1040,dependents:e.target.value})}/>
          </div>
        </div>
        ...
        <div style={styles.fieldGrid}>
          ...
          <div>
            <label style={styles.label}>Itemized Deductions (if &gt; std)</label>
        ...
        <button style={styles.calcBtn} onClick={handleCalc}>
          Calculate My Tax Bill →
        </button>
      </div>
    </>
  )

  // ── Results panel ─────────────────────────────────────────────────────────
    ...
    return (
      <div id="ts360-results" style={styles.resultsBox}>
        ...
        <div style={{ textAlign:'center', marginBottom:28 }}>
          <div style={due ? styles.dueAmt : styles.refundAmt}>
            {fmt(Math.abs(r.refundOrDue))}
          </div>
          <div style={styles.bigLabel}>
            {due ? '⚠️ Estimated Tax Due' : '🎉 Estimated Refund / Overpayment'}
          </div>
        </div>
        ...
        <div style={{ marginBottom:16 }}>
          {[
            ...
            ['SE Deduction (½ SE Tax)',   fmt(r.seDeduction)],
        ...
        <div style={due ? styles.alertRed : styles.alertGreen}>
          <div style={{ ...styles.alertTitle, color: due ? '#dc2626' : '#16a34a' }}>
            📅 Quarterly Estimated Payment: {fmt(r.quarterly)} / quarter
          </div>
        ...
        {r.qbiDed > 0 && (
          <div style={{ ...styles.alertGreen, marginTop:12 }}>
            <div style={{ ...styles.alertTitle, color:'#16a34a' }}>
              💡 QBI Deduction Saves You {fmt(r.qbiDed * 0.24)} in taxes
            </div>
            <div style={styles.alertBody}>
              Your pass-through entity qualifies for the 20% Section 199A deduction — one of the most valuable small business tax breaks available.
            </div>
          </div>
        )}
      </div>
    ...
    <div style={styles.page}>
      ...
      <div style={styles.main}>
        ...
        <p style={styles.subtitle}>
          Enter your business numbers, we calculate your share of profit (your K-1), then show you exactly<br/>
          what you owe on your personal tax return — in real time.
        </p>
        ...
        {!showManual && (
          <>
            ...
            <button
              ...
              onMouseOver={e => e.target.style.borderColor = B}
