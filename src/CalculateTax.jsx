...
return e('div',{style:{border:'2px solid '+color,borderRadius:14,overflow:'hidden',marginBottom:16}},
...
e('div',null,e('input',{value:ent.name,onChange:v=>onUpdate(idx,{...ent,name:v.target.value}),placeholder:'Business Name',style:{background:'transparent',border:'none',outline:'none',fontSize:15,fontWeight:700,color:'#fff',width:200,fontFamily:'inherit'}}),e('div',{style:{fontSize:11,color:'rgba(255,255,255,0.7)'}},ent.type))),
...
canRemove&&e('button',{onClick:()=>onRemove(idx),style:{padding:'4px 10px',background:'rgba(255,255,255,0.2)',border:'1px solid rgba(255,255,255,0.4)',borderRadius:6,fontSize:11,fontWeight:700,color:'#fff',cursor:'pointer'}},'✕ Remove'))),
...
e('div',{style:{fontSize:11,fontWeight:700,color:SL,letterSpacing:'1px'}},ent.isManual?'MANUAL ENTRY':'P&L FROM '+(ent.connectedId||'').toUpperCase()),
e('div',{style:{display:'flex',gap:6}},
!ent.isManual&&ent.connectedId&&e('button',{onClick:()=>{const t=localStorage.getItem('ts360_'+ent.connectedId+'_token'),x=localStorage.getItem('ts360_'+ent.connectedId+'_extra');if(t)fetchPnL(ent.connectedId,t,x)},style:{padding:'3px 9px',background:'#EFF6FF',border:'1px solid #BFDBFE',borderRadius:5,fontSize:11,fontWeight:600,color:B,cursor:'pointer'}},syn?'⟳':'⟳ Refresh'),
e('button',{onClick:()=>onUpdate(idx,{...ent,pnl:null,connectedId:null,isManual:false}),style:{padding:'3px 9px',background:'#FEF2F2',border:'1px solid #FECACA',borderRadius:5,fontSize:11,fontWeight:600,color:R,cursor:'pointer'}},'✕ Clear'))),
e('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,marginBottom:14}},...[['Revenue',fmt(ent.pnl.grossRevenue),G],['Expenses',fmt(ent.pnl.totalExpenses),R],['Net Profit',fmt(ent.pnl.netProfit),ent.pnl.netProfit>=0?G:R]].map(([l,v,c])=>e('div',{key:l,style:{background:'#F8FAFC',borderRadius:8,padding:'10px 14px',textAlign:'center',border:'1px solid #F1F5F9'}},e('div',{style:{fontSize:10,color:SL,marginBottom:2}},l),e('div',{style:{fontSize:17,fontWeight:800,color:c}},v)))),
e('div',{style:{display:'grid',gridTemplateColumns:'200px 1fr',gap:14,alignItems:'center'}},
e('div',null,e('label',{style:{fontSize:11,fontWeight:700,color:SL,display:'block',marginBottom:5}},'YOUR OWNERSHIP %'),
e('select',{value:ent.own,onChange:v=>onUpdate(idx,{...ent,own:v.target.value}),style:{width:'100%',padding:'9px 12px',border:'2px solid '+color,borderRadius:8,fontSize:15,fontWeight:700,color:N,background:'#fff',cursor:'pointer',fontFamily:'inherit'}},...OWN.map(([v,l])=>e('option',{key:v,value:v},l)))),
e('div',{style:{background:color,borderRadius:10,padding:'12px 20px',display:'flex',justifyContent:'space-between',alignItems:'center',color:'#fff'}},
e('div',null,e('div',{style:{fontSize:10,color:'rgba(255,255,255,0.6)',marginBottom:2}},'K-1 DISTRIBUTIVE SHARE'),e('div',{style:{fontSize:26,fontWeight:800,color:k1>=0?'#4ADE80':'#F87171'}},fmt(k1)),e('div',{style:{fontSize:10,color:'rgba(255,255,255,0.5)',marginTop:2}},fmt(ent.pnl.netProfit)+' × '+ent.own+'%')),
e('div',{style:{fontSize:11,color:'rgba(255,255,255,0.6)',maxWidth:160,textAlign:'right',lineHeight:1.4}},k1>=0?'Flows to Schedule E on your personal 1040':'Loss may offset other income on personal return'))),
ent.pnl.categories&&Object.keys(ent.pnl.categories).length>0&&e(ExpenseBreakdown,{categories:ent.pnl.categories,total:ent.pnl.totalExpenses}))))
)}
function ExpenseBreakdown({categories,total}){
const[open,setOpen]=React.useState(false)
return e('div',{style:{marginTop:12}},
e('button',{onClick:()=>setOpen(!open),style:{background:'none',border:'none',fontSize:11,fontWeight:700,color:SL,cursor:'pointer',letterSpacing:'1px',display:'flex',alignItems:'center',gap:6}},open?'▼':'►',' EXPENSE BREAKDOWN (',Object.keys(categories).length,' categories)'),
open&&e('div',{style:{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:6,marginTop:10}},...Object.entries(categories).sort((a,b)=>b[1]-a[1]).map(([cat,amt])=>e('div',{key:cat,style:{background:'#F8FAFC',borderRadius:7,padding:'7px 10px',display:'flex',justifyContent:'space-between',alignItems:'center',border:'1px solid #F1F5F9'}},e('div',null,e('div',{style:{fontSize:11,fontWeight:600,color:N}},cat),e('div',{style:{fontSize:10,color:SL}},total>0?Math.round((amt/total)*100)+'%':'')),e('div',{style:{fontSize:11,fontWeight:700,color:R}},'$'+Math.round(amt).toLocaleString())))))
}
...
const savedEntities=()=>{try{const s=sessionStorage.getItem('ts360_entities_draft');return s?JSON.parse(s):[defaultEntity()]}catch{return [defaultEntity()]}}
const[entities,setEntities]=React.useState(savedEntities)
React.useEffect(()=>{sessionStorage.setItem('ts360_entities_draft',JSON.stringify(entities))},[entities])
React.useEffect(()=>{
...
e('div',{style:{display:'flex',alignItems:'center',gap:12}},e('span',{style:{fontSize:19,fontWeight:800,color:N}},'TaxStat',e('span',{style:{color:B}},'360')),e('span',{style:{fontSize:12,background:'#EFF6FF',color:B,padding:'3px 10px',borderRadius:20,fontWeight:600}},'Step 1 of 2 — Business')),
e('div',{style:{display:'flex',gap:8}},e('button',{onClick:()=>nav('/ai-analysis'),style:{padding:'7px 14px',background:'none',border:'1px solid #E2E8F0',borderRadius:7,fontSize:13,color:SL,cursor:'pointer'}},'AI Analysis'),e('button',{onClick:()=>{localStorage.clear();nav('/')},style:{padding:'7px 14px',background:'none',border:'1px solid #E2E8F0',borderRadius:7,fontSize:13,color:SL,cursor:'pointer'}},'Sign Out'))),
e('div',{style:{maxWidth:1100,margin:'0 auto',padding:'32px 20px'}},
e('h1',{style:{fontSize:26,fontWeight:800,color:N,textAlign:'center',marginBottom:4}},'Multi-Entity K-1 Calculator'),
e('p',{style:{textAlign:'center',color:SL,fontSize:14,marginBottom:28}},'Add all your business entities. Connect each to its accounting software or enter P&L manually.'),
...entities.map((ent,idx)=>e(EntityCard,{key:idx,ent,idx,onUpdate:updateEntity,onRemove:removeEntity,canRemove:entities.length>1})),
e('button',{onClick:addEntity,style:{width:'100%',padding:'14px',background:'#fff',border:'2px dashed #CBD5E1',borderRadius:12,fontSize:14,fontWeight:700,color:SL,cursor:'pointer',marginBottom:24,display:'flex',alignItems:'center',justifyContent:'center',gap:8}},e('span',{style:{fontSize:20,lineHeight:1}},'+'),' Add Another Business Entity'),
...
e('div',null,e('div',{style:{fontSize:11,color:'rgba(255,255,255,0.45)',marginBottom:4}},'TOTAL K-1 TO SCHEDULE E'),e('div',{style:{fontSize:44,fontWeight:800,color:k1Total>=0?'#4ADE80':'#F87171'}},fmt(k1Total))),
...
!anyPnl&&e('div',{style:{textAlign:'center',padding:'32px 20px',color:SL}},e('div',{style:{fontSize:48,marginBottom:10}},'🏢'),e('div',{style:{fontSize:16,fontWeight:700,color:N,marginBottom:6}},'Add your business entities above'),e('div',{style:{fontSize:13}},'Connect accounting software or enter P&L manually for each entity'))))
)}
