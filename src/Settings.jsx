...
export default function Settings() {
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      // Keep support/admin notified as a record â best-effort, never blocks deletion.
      try {
        await fetch('https://api.web3forms.com/submit', {
          ...
          body: JSON.stringify({
            ...
            subject: 'TaxStat360 â account deleted (self-service)',
            email: email || 'unknown',
      ...
      if (e && e.status === 401) {
        ...
        setDeleteErr(
          (e && (e.body?.detail || e.message)) ||
            'Deletion failed â your account was not changed. Please try again or contact support.'
        )
      }
    }
  }
  ...
  const handleEmailChange = async () => {
    ...
    try {
      ...
      if (e instanceof ApiError) {
        ...
        setMsg('Network error â please check your connection and try again.')
      }
    }
    setLoading(false)
  }
  ...
  const handlePasswordReset = async () => {
    ...
    } catch(e) { /* intentional â always show success */ }
    setPwSent(true)
  ...
  return (
    <div style={{fontFamily:'Inter,sans-serif',minHeight:'100vh',background:'#F8FAFC'}}>
      {/* Nav
          U-01 FIX: AI Analysis nav button now conditionally shows ð for non-Pro
      ...
      <nav style={{background:'#fff',borderBottom:'1px solid #E2E8F0',padding:'0 28px',height:58,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:100,overflowX:'auto',minWidth:0}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          ...
          {/* U-01 FIX: was <NavBtn label="AI Analysis" .../> â no lock, no isPro check */}
          <button
      ...
      <div style={{maxWidth:680,margin:'0 auto',padding:'40px 20px 80px 20px'}}>
        ...
        <div style={card}>
          ...
          <div style={{borderTop:'1px solid #F1F5F9',paddingTop:20,marginBottom:20}}>
            ...
            {msg && !pwSent && <div style={{fontSize:13,color:emailSent?'#059669':'#DC2626',marginTop:8}}>{emailSent?'â ':''}{msg}</div>}
          </div>
        ...
        <div style={card}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
            <div>
              ...
              <div style={{fontSize:13,color:SL,marginTop:3}}>Billed {billingInterval.toLowerCase()} Â· Cancel anytime</div>
            </div>
          ...
          <div style={{marginBottom:24,paddingBottom:24,borderBottom:'1px solid #F1F5F9'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
              <div>
                <div style={{fontSize:13,fontWeight:600,color:N,marginBottom:3,display:'flex',alignItems:'center',gap:8}}>
                  ...
                  <span style={{
                    ...
                    {mfaEnabled ? 'â Enabled' : 'Not enabled'}
                  </span>
                </div>
              ...
              {mfaStep === 'idle' && (
                mfaEnabled ? (
                  ...
                  >
                    {mfaLoading ? 'Setting upâ¦' : 'Enable 2FA'}
                  </button>
                )
              )}
            </div>
            ...
            {mfaStep === 'setup' && mfaSetupData && (
              <div style={{marginTop:14,background:'#F8FAFC',border:'1px solid #E2E8F0',borderRadius:10,padding:'20px 20px'}}>
                <div style={{fontSize:13,fontWeight:600,color:N,marginBottom:4}}>Step 1 â Scan this QR code</div>
                ...
                {mfaSetupData.qr_code_url && (
                  <div style={{display:'flex',justifyContent:'center',marginBottom:16}}>
                    <img
                      ...
                      alt="MFA QR code â scan with your authenticator app"
                ...
                <div style={{fontSize:13,fontWeight:600,color:N,marginBottom:8}}>Step 2 â Enter the 6-digit code</div>
                <div style={{display:'flex',gap:10,alignItems:'center'}}>
                  <input
                    type="text"
                    inputMode="numeric"
                  ...
                  >
                    {mfaLoading ? 'Verifyingâ¦' : 'Verify & Enable'}
                  </button>
            ...
            {mfaStep === 'success' && (
              <div style={{marginTop:14,background:'#F0FDF4',border:'1px solid #86EFAC',borderRadius:10,padding:'20px'}}>
                <div style={{fontSize:14,fontWeight:700,color:'#059669',marginBottom:8}}>â Two-factor authentication is now active</div>
            ...
            {mfaStep === 'disable' && (
              <div style={{marginTop:14,background:'#FEF9E7',border:'1px solid #FCD34D',borderRadius:10,padding:'20px'}}>
                ...
                <div style={{display:'flex',gap:10,alignItems:'center'}}>
                  ...
                  >
                    {mfaLoading ? 'Disablingâ¦' : 'Disable 2FA'}
                  </button>
                  <button onClick={resetMfaFlow} style={{padding:'10px 14px',background:'#fff',color:SL,border:'1px solid #E2E8F0',borderRadius:8,fontWeight:600,fontSize:13,cursor:'pointer'}}>
                    Cancel
                  </button>
          ...
          <div style={{marginBottom:24}}>
            ...
            {idleTimeout !== '0' && (
              <div style={{fontSize:12,color:'#059669',marginTop:8}}>
                â You will be signed out after {idleTimeout} minutes of inactivity.
              </div>
            )}
          </div>
          ...
          <div style={{borderTop:'1px solid #F1F5F9',paddingTop:20}}>
            ...
            {loginHistory.length === 0 ? (
              <div style={{fontSize:13,color:SL,background:'#F8FAFC',border:'1px solid #E2E8F0',borderRadius:8,padding:'14px 16px'}}>
                No login history recorded yet. History is captured from this point forward â up to 10 sessions.
              </div>
            ) : (
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:20}}>
            <div>
              ...
              <div style={{fontSize:13,color:SL,lineHeight:1.5}}>
                Export a copy of all your TaxStat360 data stored on this device â tax records, session history, and account metadata â as a JSON file. You can request this at any time under CCPA and similar privacy laws.
              </div>
            </div>
            <button
            ...
            >
              {exportDone ? 'â Downloaded' : 'â¬ Export data'}
            </button>
          </div>
        </div>
