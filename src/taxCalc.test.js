...
describe('calcQBI §199A(i) OBBBA minimum deduction', () => {
  ...
  it('CHAR: does not apply floor when active QBI < $1,000', () => {
  ...
  it('CHAR: does not apply floor in pre-OBBBA tax year 2025', () => {
    const r = calcQBI(1500, 50000, 0, { status: 'single', taxYear: 2025 })
    expect(r.deduction).toBe(300)
    expect(r.limitApplied).toBe('qbi')
    expect(r.caps.min400).toBeUndefined()
  })
  it('CHAR: TI cap binds over the $400 minimum: §199A(i) floor cannot exceed taxable income', () => {
    const r = calcQBI(5000, 300, 0, { status: 'single', taxYear: 2026 })
    expect(r.deduction).toBe(60)
    expect(r.limitApplied).toBe('income')
    expect(r.caps.min400).toBe(400)
  })
  it('CHAR: no QBI deduction when TI ≤ 0, even with active QBI ≥ $1,000 (floor cannot create a deduction below zero TI)', () => {
    const r = calcQBI(5000, 0, 0, { status: 'single', taxYear: 2026 })
    expect(r.deduction).toBe(0)
    expect(r.limitApplied).toBe('none')
  })
  it('CHAR: returns regular calc when it exceeds $400 (caps.min400 still surfaces eligibility)', () => {
    const r = calcQBI(10000, 50000, 0, { status: 'single', taxYear: 2026 })
    expect(r.deduction).toBe(2000)
    expect(r.limitApplied).toBe('qbi')
    expect(r.caps.min400).toBe(400)
  })
  it('CHAR: respects activeQbi: passive-only QBI does not trigger floor', () => {
    const r = calcQBI(5000, 50000, 0, { status: 'single', taxYear: 2026, activeQbi: 500 })
    expect(r.deduction).toBe(1000)
    expect(r.limitApplied).toBe('qbi')
    expect(r.caps.min400).toBeUndefined()
  })
  it('CHAR: respects activeQbi eligibility but TI ≤ 0 still yields no deduction (floor is TI-limited)', () => {
    const r = calcQBI(500, 0, 0, { status: 'single', taxYear: 2026, activeQbi: 1000 })
    expect(r.deduction).toBe(0)
    expect(r.limitApplied).toBe('none')
  })
  it('CHAR: does not apply floor when SSTB is fully phased out (adjQBI = 0)', () => {
...
describe('calcQBI early-zero returns', () => {
  it('CHAR: returns zero deduction with qbi=0', () => {
    const r = calcQBI(0, 100000, 0, { status: 'single', taxYear: 2025 })
    expect(r.deduction).toBe(0)
    expect(r.limitApplied).toBe('none')
    expect(r.caps).toEqual({ qbi: 0, wage: null, income: 0 })
  })
  it('CHAR: returns zero deduction with negative qbi (loss carryforward case)', () => {
    const r = calcQBI(-5000, 100000, 0, { status: 'single', taxYear: 2025 })
    expect(r.deduction).toBe(0)
    expect(r.limitApplied).toBe('none')
  })
  it('CHAR: returns zero with TI ≤ 0 in pre-OBBBA year (no §199A(i) floor available)', () => {
...
describe('calcQBI above threshold with wage/UBIA data', () => {
  it('CHAR: wage cap binds: 50% wages < 20% QBI, no UBIA', () => {
  ...
  it('CHAR: UBIA cap wins over 50%-wages: 25% wages + 2.5% UBIA chosen, but qbiComponent still binds', () => {
  ...
  it('CHAR: QBI binds when wages are large (50% wages > 20% QBI)', () => {
  ...
  it('CHAR: income limit binds when capital gains shrink TI − netCapGain', () => {
    const entities = [{ box17V_sstb: false, netProfit: 50000, own: 100, box17V_wages: 80000, box17V_ubia: 0 }]
  ...
  it('CHAR: phase-in interpolation: partial wage-cap reduction when TI is mid-band', () => {
    const entities = [{ box17V_sstb: false, netProfit: 100000, own: 100, box17V_wages: 30000, box17V_ubia: 0 }]
    const r = calcQBI(100000, 220000, 0, { status: 'single', taxYear: 2025, entityQbiData: entities })
    expect(r.deduction).toBe(17730)
    expect(r.limitApplied).toBe('wage')
  })
  it('CHAR: boundary: TI just $1 above threshold falls into above-threshold branch', () => {
...
describe('calcQBI SSTB phase-in', () => {
  it('CHAR: SSTB at threshold exactly → below-threshold path → full QTB', () => {
  ...
  it('CHAR: SSTB mid phase-in: 50% applicable percentage halves both QBI and wages', () => {
    const entities = [{ box17V_sstb: true, netProfit: 50000, own: 100, box11_12: 0, box12_13: 0, box17V_wages: 30000, box17V_ubia: 0 }]
  ...
  it('CHAR: mixed SSTB + non-SSTB above phase-in: only SSTB excluded', () => {
    const entities = [
  ...
  it('CHAR: SSTB partial-ownership: own% scales k1Income for proration', () => {
  ...
  it('CHAR: SSTB sec179 (box11_12) reduces k1 income for proration', () => {
...
describe('calcQBI filing status & tax year', () => {
  it('CHAR: 2026 MFJ uses post-OBBBA threshold $403,500 and $150k phase-in', () => {
    const r = calcQBI(10000, 400000, 0, { status: 'mfj', taxYear: 2026 })
    expect(r.deduction).toBe(2000)
    expect(r.limitApplied).toBe('qbi')
    expect(r.caps.min400).toBe(400)
  })
  it('CHAR: 2026 MFJ above threshold uses wider $150k phase-in band', () => {
    const entities = [{ box17V_sstb: false, netProfit: 100000, own: 100, box17V_wages: 30000, box17V_ubia: 0 }]
    const r = calcQBI(100000, 470000, 0, { status: 'mfj', taxYear: 2026, entityQbiData: entities })
    expect(r.deduction).toBe(17783)
    expect(r.limitApplied).toBe('wage')
  })
  it('CHAR: unknown status falls back to single threshold', () => {
    const r = calcQBI(10000, 100000, 0, { status: 'civil_union', taxYear: 2025 })
    expect(r.deduction).toBe(2000)
    expect(r.limitApplied).toBe('qbi')
  })
  it('CHAR: unknown tax year falls back to 2025 thresholds', () => {
...
describe('Schedule 1 line 16 - Self-Employed Retirement', () => {
  it('CHAR: reduces AGI dollar-for-dollar', () => {
  ...
  it('CHAR: handles missing/null/undefined values without throwing', () => {
  ...
  it('CHAR: exposes selfEmpRetirementDed in the return shape', () => {
...
describe('calcTaxReturn TAX-04 — niit return is an object', () => {
  it('CHAR: niit is an object with applies, amount, explanation keys', () => {
  ...
  it('CHAR: niit.applies is false and niit.amount is 0 when AGI is below threshold', () => {
    const r = calcTaxReturn({ ...BASE, w2: 100000 })
    expect(r.niit.applies).toBe(false)
    expect(r.niit.amount).toBe(0)
    expect(r.niit.explanation).toBe('')
  })
  it('CHAR: niit.applies is true and niit.amount > 0 when AGI exceeds threshold with NII', () => {
  ...
  it('CHAR: niit.amount is 3.8% of the lesser of NII or excess AGI (lesser = NII)', () => {
    const r = calcTaxReturn({ ...BASE, w2: 220000, divInc: 30000 })
    expect(r.niit.amount).toBe(Math.round(30000 * 0.038))
    expect(r.niit.applies).toBe(true)
  })
  it('CHAR: niit.amount is 3.8% of the lesser of NII or excess AGI (lesser = excess AGI)', () => {
    const r = calcTaxReturn({ ...BASE, w2: 205000, divInc: 50000 })
    expect(r.niit.amount).toBe(Math.round(50000 * 0.038))
  })
  it('CHAR: niit.amount feeds into totalTax correctly', () => {
  ...
  it('CHAR: backward-compat niitAmount alias is a number equal to niit.amount', () => {
...
describe('calcTaxReturn TAX-01 — reasonableCompAlert', () => {
  ...
  it('CHAR: returns reasonableCompAlert in the return object', () => {
  ...
  it('CHAR: triggers when salary is below 40% of total S-Corp compensation', () => {
    const r = calcTaxReturn({ ...BASE, w2: 30000, k1Total: 140000, entities: [scorp(30000, 140000)] })
  ...
  it('CHAR: does not trigger when salary meets the 40% threshold', () => {
  ...
  it('CHAR: does not trigger when salary exceeds 40% threshold', () => {
    const r = calcTaxReturn({ ...BASE, w2: 100000, k1Total: 100000, entities: [scorp(100000, 100000)] })
    expect(r.reasonableCompAlert.triggered).toBe(false)
  })
  it('CHAR: does not trigger below the $20k de minimis floor', () => {
    const r = calcTaxReturn({ ...BASE, w2: 10000, k1Total: 8000, entities: [scorp(10000, 8000)] })
    expect(r.reasonableCompAlert.triggered).toBe(false)
  })
  it('CHAR: returns triggered=false with empty message when no S-Corp entity is present', () => {
  ...
  it('CHAR: returns triggered=false when entity list is empty', () => {
    const r = calcTaxReturn({ ...BASE, w2: 100000, entities: [] })
    expect(r.reasonableCompAlert.triggered).toBe(false)
  })
  it('CHAR: alert message includes salary amount and IRS citation', () => {
    const r = calcTaxReturn({ ...BASE, w2: 30000, k1Total: 140000, entities: [scorp(30000, 140000)] })
...
describe('calcTaxReturn TAX-06 — federalOnly flag', () => {
  it('CHAR: federalOnly is true on a basic W-2 return', () => {
    const r = calcTaxReturn({ ...BASE, w2: 100000 })
    expect(r.federalOnly).toBe(true)
  })
  it('CHAR: federalOnly is true for S-Corp entity with K-1 income', () => {
    const r = calcTaxReturn({
  ...
  it('CHAR: federalOnly is true for sole proprietor with SE tax', () => {
    const r = calcTaxReturn({
  ...
  it('CHAR: federalOnly is true even when NIIT, AMT, and CTC all fire', () => {
    const r = calcTaxReturn({
  ...
  it('CHAR: federalOnly is true regardless of tax year', () => {
...
describe('calcTaxReturn REP flag (isREP)', () => {
  it('CHAR: rentalNII is 0 when isREP=true (with §1.469-9(g) aggregation election)', () => {
    const r = calcTaxReturn({ ...BASE, rentalNet: 30000, isREP: true, rentalAggregationElection: true })
    expect(r.rentalNII).toBe(0)
  })
  it('CHAR: rentalNII equals rentalNet when isREP=false and rental is positive', () => {
    const r = calcTaxReturn({ ...BASE, rentalNet: 30000, isREP: false })
    expect(r.rentalNII).toBe(30000)
  })
  it('CHAR: REP reduces NIIT vs non-REP when AGI exceeds $200k threshold', () => {
...
describe('calcTaxReturn ISO bargain element → AMT (IRC §56(b)(3))', () => {
  it('CHAR: large ISO spread increases AMT vs baseline', () => {
  ...
  it('CHAR: isoBargainElement is ignored when hasISO=false', () => {
...
describe('calcTaxReturn ytdFactor scaling', () => {
  ...
  it('CHAR: studentLoanInt is ytdScaled and then capped at $2,500', () => {
...
describe('calcTaxReturn multi-entity portfolio', () => {
  it('CHAR: sole prop entity generates seTax; S-Corp entity does not', () => {
  ...
  it('CHAR: mixed portfolio: SE tax matches sole-prop-only amount', () => {
  ...
  it('CHAR: grossIncome sums k1Total correctly across both entities', () => {
    const r = calcTaxReturn({
      ...BASE, w2: 0, k1Total: 100000,
      entities: [
...
describe('calcTaxReturn Partnership Active vs Passive SE tax (§1402(a)(13))', () => {
  it('CHAR: Active partnership: generates SE tax', () => {
    const r = calcTaxReturn({
      ...BASE, w2: 0, k1Total: 50000,
  ...
  it('CHAR: Passive partnership: zero SE tax', () => {
    const r = calcTaxReturn({
  ...
  it('CHAR: mixed Active + Passive: SE tax computed only on Active share', () => {
    const mixed = calcTaxReturn({
  ...
  it('CHAR: legacy partnership type string normalizes to active / SE-subject', () => {
    const r = calcTaxReturn({
      ...BASE, w2: 0, k1Total: 100000,
...
describe('calcTaxReturn NIIT — F-01 qualDiv no-double-count guard', () => {
  it('CHAR: NIIT amount is unchanged when qualDiv is set vs unset (with same divInc)', () => {
  ...
  it('CHAR: NIIT scales with divInc but NOT additionally with qualDiv', () => {
...
describe('calcTaxReturn NIIT — F-01-followup-A stGain in NII base', () => {
  ...
  it('CHAR: no NIIT when AGI is below the $200k single threshold', () => {
    const r = calcTaxReturn({ ...BASE, w2: 100000, stGain: 50000 })
    expect(r.niit.amount).toBe(0)
    expect(r.niit.applies).toBe(false)
  })
  it('CHAR: NIIT is capped by NII when AGI barely exceeds threshold', () => {
...
describe('calcQBI SSTB — F-03 no double-ownership guard', () => {
  it('CHAR: SSTB deduction depends on e.k1, not on own% (same k1 + different own% = same result)', () => {
    const entityOwn50 = [{
...
describe('calcTaxReturn §469 PAL — non-REP passive rental limitation', () => {
  it('CHAR: non-REP low AGI ($80k): $25k special allowance fully available', () => {
  ...
  it('CHAR: non-REP mid AGI ($120k): $25k allowance prorated to $15k', () => {
    const r = calcTaxReturn({ ...BASE, w2: 120000, rentalNet: -20000, isREP: false })
    expect(r.grossIncome).toBe(105000)
    expect(r.palSuspendedRental).toBe(5000)
  })
  it('CHAR: non-REP: rental income (positive) is unaffected by PAL gating', () => {
  ...
  it('CHAR: MFS: $0 allowance by default (§469(i)(5)(B))', () => {
  ...
  it('CHAR: PAL phase-out boundary: AGI exactly $100k gets full $25k allowance', () => {
    const r = calcTaxReturn({ ...BASE, w2: 100000, rentalNet: -30000, isREP: false })
    expect(r.palSuspendedRental).toBe(5000)
    expect(r.grossIncome).toBe(75000)
  })
  it('CHAR: PAL phase-out boundary: AGI exactly $150k gets $0 allowance', () => {
...
describe('calcTaxReturn §461(l) — excess business loss limitation', () => {
  it('CHAR: business loss below threshold: no EBL, full loss allowed', () => {
    const r = calcTaxReturn({ ...BASE, w2: 300000, k1Total: -200000, isREP: false })
    expect(r.ebl).toBe(0)
    expect(r.grossIncome).toBe(100000)
  })
  it('CHAR: business loss above threshold: excess becomes EBL carryforward', () => {
    const r = calcTaxReturn({ ...BASE, w2: 500000, k1Total: -400000, isREP: false })
    expect(r.ebl).toBe(87000)
    expect(r.grossIncome).toBe(187000)
  })
  it('CHAR: W-2 income is NOT counted as business income in EBL computation', () => {
  ...
  it('CHAR: REP rental losses count toward EBL business bucket (elected/nonpassive)', () => {
    const r = calcTaxReturn({ ...BASE, w2: 500000, k1Total: -200000, rentalNet: -200000, isREP: true, rentalAggregationElection: true })
    expect(r.ebl).toBe(87000)
  })
  it('CHAR: Pass 5 scenario: S-Corp -$343k + REP rental -$85k (elected) → EBL $115k', () => {
    const r = calcTaxReturn({
      ...BASE,
  ...
  it('CHAR: MFJ: higher threshold ($626k) applies', () => {
    const r = calcTaxReturn({ ...BASE, w2: 800000, k1Total: -700000, status: 'mfj', isREP: false })
    expect(r.ebl).toBe(74000)
  })
  it('CHAR: combined PAL + EBL: both limitations apply in a realistic mixed scenario', () => {
  ...
  it('CHAR: business gain offsets business loss before threshold check', () => {
...
describe('calcTaxReturn §469(i) — isActiveParticipant flag', () => {
  it('CHAR: passive investor (isActiveParticipant=false): rental losses fully suspended', () => {
    const r = calcTaxReturn({ ...BASE, w2: 80000, rentalNet: -30000, isREP: false, isActiveParticipant: false })
    expect(r.palSuspendedRental).toBe(30000)
    expect(r.grossIncome).toBe(80000)
  })
  it('CHAR: active participant (default): $25k allowance applies normally', () => {
    const r = calcTaxReturn({ ...BASE, w2: 80000, rentalNet: -30000, isREP: false })
    expect(r.palSuspendedRental).toBe(5000)
    expect(r.grossIncome).toBe(55000)
  })
...
describe('calcTaxReturn §199A SSTB loss — qbiCarryforward exclusion above phase-out', () => {
  ...
  it('CHAR: below threshold: SSTB loss fully carries forward as QBI loss', () => {
    const r = calcTaxReturn({
  ...
  it('CHAR: fully above phase-out: SSTB loss excluded from qbiCarryforward', () => {
    const r = calcTaxReturn({
  ...
  it('CHAR: partial phase-out: SSTB loss scaled by applicable percentage', () => {
    const r = calcTaxReturn({
  ...
  it('CHAR: non-SSTB loss above threshold: still carries forward', () => {
    const r = calcTaxReturn({
      ...BASE, w2: 350000, k1Total: -100000,
...
describe('calcQBI/calcTaxReturn F-M02 — ownPct() 0% ownership', () => {
  ...
  it('CHAR: own=0 (numeric zero) also contributes $0 — no phantom income', () => {
    const entities = [{ type: 'S Corporation', netProfit: 100000, own: 0, k1: 0, box17V_wages: 0, box17V_ubia: 0 }]
    const r = calcTaxReturn({ ...BASE, k1Total: 0, entities })
    expect(r.grossIncome).toBe(100000)
    expect(r.qbi).toBe(0)
  })
  it('CHAR: own=null defaults to 100% — backward compatible', () => {
  ...
  it('CHAR: own=undefined defaults to 100% — backward compatible', () => {
    const entities = [{ type: 'S Corporation', netProfit: 100000, k1: 100000, box17V_wages: 0, box17V_ubia: 0 }]
    const r = calcTaxReturn({ ...BASE, k1Total: 100000, entities })
    expect(r.grossIncome).toBeGreaterThan(100000)
  })
  it('CHAR: 0% SSTB entity does not generate phantom §199A exclusion', () => {
  ...
  it('CHAR: 0%-owned S-Corp does not trigger reasonable comp alert', () => {
...
describe('calcAMT PASS4B-01 — MFS 2026 bracket26_28 typo fix', () => {
  ...
  it('CHAR: PASS4B-01 pinned: calcAMT(MFS, 2026, TI=300k) = 52,435', () => {
    const result = calcAMT(MFS_2026_BASE)
    expect(result).toBe(52435)
  })
  it('CHAR: PASS4B-01 directional: MFS AMT > result under simulated pre-fix single threshold', () => {
    const result = calcAMT(MFS_2026_BASE)
    expect(result).toBeGreaterThan(0)
    expect(Number.isFinite(result)).toBe(true)
  })
  it('CHAR: PASS4B-01 smoke: calcTaxReturn with ISO-heavy MFS 2026 filer — finite AMT', () => {
    const r = calcTaxReturn({
      ...BASE,
  ...
  it('CHAR: PASS4B-01 prior-year guard: 2024/2025 MFS AMT unaffected by 2026 fix', () => {
...
describe('calcTaxReturn F6 — per-property material participation + §1.469-9(g) aggregation', () => {
  ...
  it('CHAR: REP without the §1.469-9(g) election: rental stays passive, loss suspended', () => {
  ...
  it('CHAR: REP + §1.469-9(g) aggregation election: whole portfolio nonpassive', () => {
  ...
  it('CHAR: REP + Step-2 materiallyParticipates=true (no election): nonpassive', () => {
  ...
  it('CHAR: REP + Step-2 materiallyParticipates=false: passive, suspended (AGI > $150k → $0 allowance)', () => {
  ...
  it('CHAR: aggregation election overrides a false Step-2 MP answer', () => {
  ...
  it('CHAR: non-REP + Step-2 MP=true: still passive (§469(c)(2) per se), §469(i) allowance applies', () => {
  ...
  it('CHAR: REP non-participating positive rental is passive income → in NII base', () => {
  ...
  it('CHAR: REP materially-participating positive rental is nonpassive → excluded from NII', () => {
  ...
  it('CHAR: Step-1 per-entity materiallyParticipates=false (REP): that property passive & suspended', () => {
    const r = calcTaxReturn({
  ...
  it('CHAR: §469(i) allowance applies to a Step-1 rental entity even with NO Step-2 lump', () => {
  ...
  it('CHAR: Step-1 per-entity materiallyParticipates=true (REP): that property nonpassive', () => {
    const r = calcTaxReturn({
      ...BASE, w2: 200000, k1Total: -50000, isREP: true,
  ...
  it('CHAR: tri-state: REP with an UNANSWERED Step-1 rental (undefined) → passive-pending (suspended)', () => {
...
describe('calcTaxReturn F5 — reasonable-comp advisory framing', () => {
  ...
  it('CHAR: still triggers on the same 35–45% heuristic and keeps {triggered, ratio, message} contract', () => {
    const r = calcTaxReturn({ ...BASE, w2: 30000, k1Total: 140000, entities: [scorp(30000, 140000)] })
    expect(r.reasonableCompAlert.triggered).toBe(true)
    expect(r.reasonableCompAlert.ratio).toBe(18)
  ...
  it('CHAR: message is advisory: leads with informational framing and cites §1.162-7', () => {
    const r = calcTaxReturn({ ...BASE, w2: 30000, k1Total: 140000, entities: [scorp(30000, 140000)] })
  ...
  it('CHAR: retains required substrings: 35–45%, Watson v. Commissioner, and the salary amount', () => {
    const r = calcTaxReturn({ ...BASE, w2: 30000, k1Total: 140000, entities: [scorp(30000, 140000)] })
    const m = r.reasonableCompAlert.message
...
describe('calcTaxReturn unrecaptured 1250 - subset of net capital gain, not added income', () => {
  it('CHAR: entering unrecaptured 1250 does not inflate income (Verela 2024 double-count guard)', () => {
    const base2024 = {
